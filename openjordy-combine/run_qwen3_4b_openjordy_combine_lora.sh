...
SAVE_CKPT=${SAVE_CKPT:-${REPO_ROOT}/ckpt/qwen3-4b-openjordy-combine-lora}
...
export OPENJORDY_RECORD_FILE="${SCRIPT_DIR}/results/qwen3_4b_lora_record.jsonl"
export TP="${TP:-1}"
...
if [ "${USE_WANDB}" = "1" ] && [ -n "${WANDB_KEY_VALUE}" ]; then
  WANDB_ARGS=(
    ...
    --wandb-group qwen3-4b-openjordy-combine-lora
...
RUNTIME_ENV_JSON="{
  \"env_vars\": {
    \"PYTHONPATH\": \"${SCRIPT_DIR}:${SCRIPT_DIR}/../openjordy-opd:${SLIME_ROOT}\",
