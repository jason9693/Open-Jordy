...
from openjordy_opd_api_server import OpenJordyOPDAPIServer, generate, reward_func  # noqa: F401
from slime.utils.types import Sample

logger = logging.getLogger(__name__)


class OpenJordyCombineAPIServer(OpenJordyOPDAPIServer):
    ...
    async def _submit_turn_sample(
        ...
        logger.info(
            "[OpenJordy-Combine] submitted %s sample session=%s index=%d "
    ...
    async def _submit_rl_turn_sample(
        ...
        logger.info(
            "[OpenJordy-Combine] submitted RL sample session=%s index=%d "
        ...
        for turn_num in sorted(list(pending.keys())):
            ...
            if task is None:
                if force_drop_without_next_state:
                    ...
                    logger.info(
                        "[OpenJordy-Combine] dropped session=%s turn=%d (no next_state)",
                        session_id,
                        turn_num,
                    )
                continue
            ...
            try:
                ...
                logger.warning(
                    "[OpenJordy-Combine] evaluation task failed session=%s turn=%d: %s",
                    session_id,
                    turn_num,
                    e,
                )
