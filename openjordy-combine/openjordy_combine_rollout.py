...
from openjordy_combine_api_server import OpenJordyCombineAPIServer
...
class AsyncRolloutWorker:
    def __init__(self, args, data_buffer):
        ...
        self._server = OpenJordyCombineAPIServer(
            args=args,
    ...
    def pause_submission(self):
        if self._submission_enabled.is_set():
            ...
            print("[OpenJordyCombineWorker] submission paused")

    def resume_submission(self):
        if not self._submission_enabled.is_set():
            self._submission_enabled.set()
            print("[OpenJordyCombineWorker] submission resumed")
...
async def _drain_output_queue(args, worker: AsyncRolloutWorker) -> list[list[Sample]]:
    ...
    while len(data) < target_data_size:
        ...
        if time.time() - last_progress > 30:
            print(
                f"[OpenJordyCombineWorker] waiting for combine samples: {len(data)}/{target_data_size}, "
    ...
    print(f"[OpenJordyCombineWorker] drained {len(data)} groups in {time.time() - start:.2f}s", flush=True)
    return data


def generate_rollout_openjordy_combine(args, rollout_id, data_buffer, evaluation=False):
    ...
    if eval_scores:
        ...
        print(
            f"[OpenJordyCombineWorker] prm_eval_score={extra_metrics['rollout/prm_eval_score']:.4f} "
            f"(n={len(eval_scores)})",
            flush=True,
        )
