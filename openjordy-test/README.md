# OpenJordy-Test: End-to-End Evaluation for OpenJordy-RL Training Methods

This directory contains an automated evaluation suite that tests the **real-world effectiveness** of models trained with the three OpenJordy-RL optimization methods:

| Method | Directory | Signal Type |
|---|---|---|
| **Combined (RL + OPD)** | `openjordy-combine/` | Weighted combination of both signals |
| **Binary RL (GRPO)** | `openjordy-rl/` | Scalar reward (+1/−1/0) via PRM majority voting |
| **On-Policy Distillation (OPD)** | `openjordy-opd/` | Token-level directional signal from hindsight hints |

We recommend using **Combined (RL + OPD)**.

## What Does This Test Do?

The evaluation simulates a realistic multi-turn agentic workflow using **GSM8K math problems** as the task domain. An external LLM (the "user") interacts with the OpenJordy agent (your trained model) through the OpenJordy gateway API, testing whether the agent can:
...
An external LLM role-plays as a **lazy student** who asks the OpenJordy agent to do their homework. For each GSM8K problem:

1. The problem is written to `homework/i.txt` in the OpenJordy workspace.
...
```
┌─────────────────────┐         ┌───────────────────────────────┐
│   External LLM      │         │      OpenJordy RL Server       │
│  (Student/Teacher)   │         │   (your trained model)        │
│  Port 30001         │         │   Port 30000                  │
│  via launch_user_   │         │   via openjordy-rl/opd/combine │
...
- A running OpenJordy environment (see the [main README](../README.md))
...
### Step 2: Start the OpenJordy RL Server
...
> **Eval mode:** To enable evaluation logging with W&B, set `OPENJORDY_EVAL_MODE=1` and provide your W&B key via `WANDB_KEY` before launching. This is already the default in the OPD and Combine scripts.
...
```bash
# Required
export OPENJORDY_GATEWAY_TOKEN="your-gateway-token"
export OPENAI_API_KEY="your-external-llm-api-key"

# Optional (defaults shown)
export OPENJORDY_GATEWAY_URL="http://localhost:18789"
export OPENJORDY_WORKSPACE="$HOME/.openjordy/workspace"
...
2. For each problem, run a multi-turn conversation where the student LLM asks the OpenJordy agent to solve it.
...
This will:
1. For each problem, the teacher LLM asks the OpenJordy agent to read the student's submission, compare with the ground truth, and write grading comments.
...
|---|---|---|---|
| `OPENJORDY_GATEWAY_TOKEN` | Yes | — | Auth token for the OpenJordy gateway |
| `OPENAI_API_KEY` | Yes | — | API key for the external LLM (student/teacher) |
| `OPENJORDY_GATEWAY_URL` | No | `http://localhost:18789` | OpenJordy gateway base URL |
| `OPENJORDY_WORKSPACE` | No | `~/.openjordy/workspace` | Path to the OpenJordy workspace directory |
