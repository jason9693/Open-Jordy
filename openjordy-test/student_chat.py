"""
AI-to-OpenJordy GSM8K homework runner.

Loads problems from a GSM8K JSON file, writes each problem into the OpenJordy
...
def run_one_problem(
    ...
    for turn in range(max_turns):
        ...
        print(f"  {'='*56}")
        print(f"  >> Student -> OpenJordy:\n  {student_msg}\n")

        time.sleep(1)

        conversation_history.append({"role": "assistant", "content": student_msg})

        openjordy_reply = send_to_openjordy(gateway_url, gateway_token, student_msg, session_user)
        print(f"  << OpenJordy -> Student:\n  {openjordy_reply}\n")
...
def main():
    parser = argparse.ArgumentParser(description="GSM8K homework runner via OpenJordy")
    ...
    workspace = os.environ.get(
        "OPENJORDY_WORKSPACE",
        os.path.expanduser("~/.openjordy/workspace"),
    )
