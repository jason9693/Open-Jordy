"""
AI-to-OpenJordy GSM8K homework grading runner.

The external LLM plays a teacher who uses OpenJordy to grade student homework.
...
and ground truth answer to OpenJordy, asks it to grade, reviews the feedback,
and then has it appended to the file.

Required environment variables:
  OPENJORDY_GATEWAY_TOKEN  - Your OpenJordy gateway auth token
  OPENAI_API_KEY          - API key for the external (teacher) LLM

Optional environment variables:
  OPENJORDY_GATEWAY_URL    - Gateway base URL (default: http://localhost:18789)
  OPENJORDY_WORKSPACE      - Workspace path (default: ~/.openjordy/workspace)
...
def send_to_openjordy(
    gateway_url: str,
    token: str,
    message: str,
    session_user: str,
) -> str:
    """Send a message to OpenJordy, maintaining session via the user field."""
    resp = requests.post(
...
def run_one_grading(
    ...
    for turn in range(max_turns):
        ...
        print(f"  {'='*56}")
        print(f"  >> Teacher -> OpenJordy:\n  {teacher_msg}\n")

        time.sleep(1)

        conversation_history.append({"role": "assistant", "content": teacher_msg})

        openjordy_reply = send_to_openjordy(gateway_url, gateway_token, teacher_msg, session_user)
        print(f"  << OpenJordy -> Teacher:\n  {openjordy_reply}\n")
...
def main():
    parser = argparse.ArgumentParser(description="GSM8K homework grading runner via OpenJordy")
    ...
    gateway_token = get_env_or_exit("OPENJORDY_GATEWAY_TOKEN")
    gateway_url = os.environ.get("OPENJORDY_GATEWAY_URL", "http://localhost:18789").rstrip("/")
