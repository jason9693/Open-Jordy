...
from openjordy_opd_api_server import OpenJordyOPDAPIServer
...
class AsyncRolloutWorker:
    def __init__(self, args, data_buffer):
        ...
        self._server = OpenJordyOPDAPIServer(
            args=args,
...
async def _drain_output_queue(args, worker: AsyncRolloutWorker) -> list[list[Sample]]:
    ...
    while len(data) < target_data_size:
        ...
        if time.time() - last_progress > 30:
            print(
                f"[OpenJordyOPDWorker] waiting for valid OPD samples: {len(data)}/{target_data_size}, "
    ...
    print(f"[OpenJordyOPDWorker] drained {len(data)} groups in {time.time() - start:.2f}s", flush=True)
    return data


def generate_rollout_openjordy_opd(args, rollout_id, data_buffer, evaluation=False):
    ...
    if eval_scores:
        ...
        print(
            f"[OpenJordyOPDWorker] prm_eval_score={extra_metrics['rollout/prm_eval_score']:.4f} "
            f"(n={len(eval_scores)})",
            flush=True,
        )
