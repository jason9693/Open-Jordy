...
```bash
cd slime
bash ../openjordy-opd/run_qwen3_4b_openjordy_opd_topk.sh
```
...
openjordy-opd/
├── README.md
├── run_qwen3_4b_openjordy_opd.sh            # Token-level OPD (default)
├── run_qwen3_4b_openjordy_opd_topk.sh       # Top-K custom-loss path
├── topk_distillation_loss.py               # Reverse-KL top-K loss (external custom loss)
├── openjordy_opd_api_server.py              # Async judge + teacher query + sample submission
├── openjordy_opd_rollout.py                 # Rollout bridge to SLIME trainer
