...
SAVE_CKPT=${SAVE_CKPT:-/absolute/path/to/OpenJordy-RL/ckpt/qwen3-4b-openjordy-opd}
...
ROLLOUT_ARGS=(
   ...
   --rollout-function-path openjordy_opd_rollout.generate_rollout_openjordy_opd
...
CUSTOM_ARGS=(
   --custom-generate-function-path openjordy_opd_api_server.generate
   --custom-rm-path openjordy_opd_api_server.reward_func
)
...
if [ "${USE_WANDB}" = "1" ] && [ -n "${WANDB_KEY_VALUE}" ]; then
  WANDB_ARGS=(
    ...
    --wandb-group qwen3-4b-openjordy-opd
...
RUNTIME_ENV_JSON="{
  \"env_vars\": {
    \"PYTHONPATH\": \"/absolute/path/to/OpenJordy-RL/Megatron-LM/:${SCRIPT_DIR}:${SLIME_ROOT}\",
