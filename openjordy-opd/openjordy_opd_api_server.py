...
class OpenJordyOPDAPIServer:
    def __init__(self, args, output_queue: queue.Queue, submission_enabled: threading.Event):
        ...
        self._teacher_lp_max_concurrency = int(os.getenv("OPENJORDY_OPD_TEACHER_LP_MAX_CONCURRENCY", "3"))
        ...
        if self._prm_enabled:
            ...
            logger.info("[OpenJordy-OPD] PRM enabled: url=%s m=%d", self._prm_url, self._prm_m)

        self._record_file = os.getenv("OPENJORDY_RECORD_FILE", "") if os.getenv("OPENJORDY_RECORD_ENABLED", "0") == "1" else ""
        if self._record_file:
            os.makedirs(os.path.dirname(self._record_file), exist_ok=True)
            open(self._record_file, "w").close()
            logger.info("[OpenJordy-OPD] record file initialized (cleared): %s", self._record_file)

        self._prm_record_file = os.getenv("OPENJORDY_PRM_RECORD_FILE", "")
        ...
        if self._prm_record_file:
            ...
            logger.info("[OpenJordy-OPD] PRM record file initialized (cleared): %s", self._prm_record_file)

        self._eval_mode = os.getenv("OPENJORDY_EVAL_MODE", "0") == "1"
        self._eval_scores: list[float] = []
        self._eval_scores_lock = threading.Lock()
        if self._eval_mode:
            logger.info("[OpenJordy-OPD] eval mode enabled: will compute RL-style PRM scores")
    ...
    def _build_app(self) -> FastAPI:
        app = FastAPI(title="OpenJordy OPD Proxy")
        ...
        async def chat_completions(
            ...
            owner: OpenJordyOPDAPIServer = request.app.state.owner
    ...
    def _flush_pending_record(self, session_id: str, next_state: dict[str, Any] | None):
        ...
        if self._record_file:
            try:
                ...
                logger.warning("[OpenJordy-OPD] failed to write record: %s", e)
    ...
    def _append_prm_record(self, record: dict[str, Any]):
        ...
        try:
            ...
            logger.warning("[OpenJordy-OPD] failed to write PRM record: %s", e)
    ...
    async def _query_judge_once(self, judge_prompt: str, vote_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning("[OpenJordy-OPD] judge query failed (vote %d): %s", vote_id, e)
            return {"vote_id": vote_id, "score": None, "hint": "", "raw": ""}
    ...
    async def _query_prm_eval_once(self, prompt_text: str, vote_id: int) -> int | None:
        ...
        try:
            ...
            logger.warning("[OpenJordy-OPD] PRM eval query failed (vote %d): %s", vote_id, e)
            return None
    ...
    async def _opd_evaluate(self, session_id: str, turn_num: int, turn_data: dict[str, Any], next_state: dict[str, Any]) -> dict[str, Any]:
        ...
        if self._eval_mode:
            ...
            logger.info(
                "%s[OpenJordy-OPD] PRM eval session=%s turn=%d eval_votes=%s → eval_score=%.1f%s",
        ...
        if selected is None:
            logger.info(
                "%s[OpenJordy-OPD] session=%s turn=%d no valid hint (votes=%s), sample dropped%s",
                _CYAN,
                session_id,
                turn_num,
                votes_display,
        ...
        logger.info(
            "%s[OpenJordy-OPD] session=%s turn=%d accepted hint_len=%d votes=%s%s",
            _CYAN,
            session_id,
            turn_num,
            len(hint),
    ...
    async def _handle_request(
        ...
        async with httpx.AsyncClient(timeout=None) as client:
            ...
            if sglang_resp.status_code != 200:
                logger.error("[OpenJordy-OPD] SGLang returned %d: %s", sglang_resp.status_code, sglang_resp.text[:1000])
        ...
        logger.info(
            "%s[OpenJordy-OPD] [%s] session=%s prompt_msgs=%d%s",
            _YELLOW,
            turn_type,
            session_id,
            len(messages),
        ...
        logger.info(
            "%s[OpenJordy-OPD] [%s] session=%s thinking=%d chars, response:\n%s%s",
            _RED,
            turn_type,
            session_id,
            len(reasoning),
        ...
        if tool_calls:
            logger.info("[OpenJordy-OPD] tool_calls: %s", str(tool_calls)[:500])
        ...
        if turn_type == "main":
            ...
            if not response_ids and not response_text.strip():
                logger.info("[OpenJordy-OPD] MAIN session=%s -> empty response, skipping", session_id)
            ...
            logger.info(
                "[OpenJordy-OPD] MAIN session=%s turn=%d prompt_tokens=%d response_tokens=%d",
                session_id,
                turn_num,
                len(prompt_ids),
                len(response_ids),
            ...
            logger.info("[OpenJordy-OPD] SIDE session=%s -> skipped (no training data)", session_id)
        ...
        if session_done:
            ...
            logger.info("[OpenJordy-OPD] session=%s done -> cleaned up", session_id)
        ...
        for turn_num in sorted(list(pending.keys())):
            ...
            if task is None:
                if force_drop_without_next_state:
                    ...
                    logger.info(
                        "[OpenJordy-OPD] dropped session=%s turn=%d (no next_state -> no hint teacher)",
                        session_id,
                        turn_num,
                    )
                continue
            ...
            try:
                ...
                logger.warning("[OpenJordy-OPD] opd task failed session=%s turn=%d: %s", session_id, turn_num, e)
                if self._eval_mode:
                    with self._eval_scores_lock:
                        self._eval_scores.append(0.0)
                continue
    ...
    async def _submit_turn_sample(self, turn_data: dict[str, Any], session_id: str, opd_result: dict[str, Any]):
        ...
        logger.info(
            "[OpenJordy-OPD] submitted sample session=%s index=%d prompt_len=%d response_len=%d hint_len=%d",
            session_id,
            sample.index,
            len(prompt_ids),
            len(response_ids),
    ...
    def purge_record_files(self):
        for path, label in [
            ...
            try:
                open(path, "w").close()
                logger.info("[OpenJordy-OPD] %s file purged: %s", label, path)
            except OSError as e:
                logger.warning("[OpenJordy-OPD] failed to purge %s file: %s", label, e)
    ...
    def _task_done_cb(task: asyncio.Task):
        ...
        exc = task.exception()
        if exc is not None:
            logger.error("[OpenJordy-OPD] background task failed: %s", exc, exc_info=exc)
    ...
    def _wait_for_sglang_ready(self):
        while True:
            try:
                ...
                pass
            time.sleep(3)
        logger.info("[OpenJordy-OPD] policy server ready")
        ...
        if self._prm_enabled and self._prm_url:
            ...
            while True:
                try:
                    ...
                    pass
                time.sleep(3)
            logger.info("[OpenJordy-OPD] PRM/teacher server ready")

        time.sleep(8)
        banner = (
            f"\n{'=' * 70}\n"
            f"  [OpenJordy-OPD] model is ready\n"
