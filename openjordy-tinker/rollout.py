...
from api_server import OpenJordyRLServer, OpenJordyOPDServer, OpenJordyCombineServer
...
class RolloutWorker:
    ...
    def __init__(
        ...
        if method == "rl":
            self._server = OpenJordyRLServer(
            ...
            self._server = OpenJordyOPDServer(
                config=config, output_queue=self.output_queue,
                submission_enabled=self._submission_enabled,
                sampling_client=sampling_client, opd_scorer=scorer,
            )
        elif method == "combine":
            self._server = OpenJordyCombineServer(
                config=config, output_queue=self.output_queue,
                submission_enabled=self._submission_enabled,
