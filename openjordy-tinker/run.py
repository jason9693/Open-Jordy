#!/usr/bin/env python3
"""Entry point for OpenJordy Tinker training.
...
def parse_args() -> TinkerConfig:
    parser = argparse.ArgumentParser(description="OpenJordy training on Tinker (RL / OPD / Combined)")
    ...
    parser.add_argument("--w-opd", type=float, default=float(os.getenv("OPENJORDY_COMBINE_W_OPD", "1.0")),
                        help="OPD advantage weight (combine method only)")
    parser.add_argument("--w-rl", type=float, default=float(os.getenv("OPENJORDY_COMBINE_W_RL", "1.0")),
    ...
    parser.add_argument("--wandb-project", default=os.getenv("WANDB_PROJECT", "openjordy-tinker"))
