...
Unified training framework for OpenJordy on [Tinker](https://tinker.build) cloud infrastructure. Supports three training methods through a single entry point:
...
- **RolloutWorker** (`rollout.py`): Spins up a local OpenAI-compatible API server that forwards requests to the Tinker policy model. External environments (OpenJordy tasks) connect to this server. Completed sessions are queued for scoring and training.
...
- **API Server** (`api_server.py`): Base class `_BaseServer` provides shared infrastructure (Tinker forwarding, auth, streaming, tokenization, record management). Three subclasses handle method-specific logic:
  - `OpenJordyRLServer` — PRM scoring with at-least-one guarantee
  - `OpenJordyOPDServer` — Hint judge + teacher log-probs, drops turns without next_state
  - `OpenJordyCombineServer` — Three-way dispatch (opd+rl / opd-only / rl-only)
...
|------|---------|---------|--------|-------------|
| `--w-opd` | `OPENJORDY_COMBINE_W_OPD` | `1.0` | combine | OPD advantage weight |
| `--w-rl` | `OPENJORDY_COMBINE_W_RL` | `1.0` | combine | RL advantage weight |
