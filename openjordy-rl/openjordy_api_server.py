...
class OpenJordyAPIServer:
    """Proxy between OpenJordy and SGLang for RL training data collection.

    OpenJordy sends ``X-Session-Id`` and ``X-Turn-Type`` headers with every
    ...
    def __init__(self, args, output_queue: queue.Queue, submission_enabled: threading.Event):
        ...
        if self._prm_enabled:
            ...
            logger.info("[OpenJordy] PRM enabled: url=%s m=%d", self._prm_url, self._prm_m)
        ...
        if self._record_file:
            ...
            logger.info("[OpenJordy] record file initialized (cleared): %s", self._record_file)
        ...
        if self._prm_record_file:
            ...
            logger.info("[OpenJordy] PRM record file initialized (cleared): %s", self._prm_record_file)
    ...
    def _build_app(self) -> FastAPI:
        app = FastAPI(title="OpenJordy SLIME Proxy")
        ...
        async def chat_completions(
            ...
            owner: OpenJordyAPIServer = request.app.state.owner
    ...
    def _flush_pending_record(self, session_id: str, next_state):
        ...
        if next_state:
            ...
            logger.info(
                f"{_GREEN}[OpenJordy] session={session_id} turn={rec['turn']} "
        ...
        if self._record_file:
            try:
                ...
                logger.warning("[OpenJordy] failed to write record: %s", e)
    ...
    def _append_prm_record(self, session_id: str, turn_num: int,
        ...
        try:
            ...
            logger.warning("[OpenJordy] failed to write PRM record: %s", e)
    ...
    def purge_record_files(self):
        ...
        for path, label in [
            ...
            try:
                open(path, "w").close()
                logger.info("[OpenJordy] %s file purged: %s", label, path)
            except OSError as e:
                logger.warning("[OpenJordy] failed to purge %s file: %s", label, e)
    ...
    async def _query_prm_once(self, judge_prompt: str, vote_id: int) -> tuple[int | None, str]:
        ...
        try:
            ...
            logger.warning("[OpenJordy] PRM query failed (vote %d): %s", vote_id, e)
            return None, ""
    ...
    async def _prm_evaluate(self, session_id: str, turn_num: int,
        ...
        logger.info(
            f"{_CYAN}[OpenJordy] PRM session={session_id} turn={turn_num} "
    ...
    async def _handle_request(
        ...
        async with httpx.AsyncClient(timeout=None) as client:
            ...
            if sglang_resp.status_code != 200:
                logger.error("[OpenJordy] SGLang returned %d: %s", sglang_resp.status_code, sglang_resp.text[:1000])
                sglang_resp.raise_for_status()
            try:
                output = sglang_resp.json()
            except Exception:
                logger.error("[OpenJordy] SGLang non-JSON body: %s", sglang_resp.text[:1000])
                raise
        ...
        logger.info(
            f"{_YELLOW}[OpenJordy] [{turn_type}] session={session_id} "
            f"prompt_msgs={len(messages)}{_RESET}"
        )
        logger.info(
            f"{_RED}[OpenJordy] [{turn_type}] session={session_id} "
            f"thinking={len(reasoning)} chars, response:\n{content}{_RESET}"
        )
        if tool_calls:
            logger.info("[OpenJordy] tool_calls: %s", json.dumps(tool_calls, ensure_ascii=False)[:500])
        ...
        if turn_type == "main":
            ...
            if full_text.startswith(prompt_text):
                ...
                logger.warning("[OpenJordy] prompt_text is not a prefix of full_text, using full_text as response")
            ...
            if not response_ids and not response_text.strip():
                logger.info("[OpenJordy] MAIN session=%s → empty response, skipping", session_id)
            ...
            logger.info(
                "[OpenJordy] MAIN session=%s turn=%d prompt_tokens=%d response_tokens=%d",
            ...
            logger.info("[OpenJordy] SIDE session=%s → skipped (no training data)", session_id)
        ...
        if session_done:
            ...
            logger.info("[OpenJordy] session=%s done → cleaned up (effective_samples=%d)", session_id, eff)
    ...
    async def _submit_turn_sample(self, turn_data: dict[str, Any], session_id: str,
        ...
        if exclude and has_next_state and self._session_effective.get(session_id, 0) == 0:
            exclude = False
            logger.info("[OpenJordy] promoting session=%s turn with score=0 → loss_mask=1 (at-least-one guarantee)", session_id)
        ...
        logger.info(
            "[OpenJordy] submitted sample session=%s index=%d score=%.1f exclude=%s prompt_len=%d response_len=%d",
    ...
    def _task_done_cb(task: asyncio.Task):
        ...
        exc = task.exception()
        if exc is not None:
            logger.error("[OpenJordy] background task failed: %s", exc, exc_info=exc)
    ...
    def _wait_for_sglang_ready(self):
        while True:
            try:
                ...
                pass
            time.sleep(3)
        logger.info("[OpenJordy] policy server ready")
        ...
        if self._prm_enabled and self._prm_url:
            ...
            while True:
                try:
                    ...
                    pass
                time.sleep(3)
            logger.info("[OpenJordy] PRM server ready")
        ...
        banner = (
            f"\n{'=' * 70}\n"
            f"  [OpenJordy] your model is fired up and ready to roll\n"
