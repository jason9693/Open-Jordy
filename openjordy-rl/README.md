...
The policy model is deployed as an OpenAI-compatible chat proxy. External environments (e.g. OpenJordy) send multi-turn conversations through this proxy. For each **main-line turn**, the system:
...
openjordy-rl/
├── README.md
├── run_qwen3_4b_openjordy_rl.sh     # Launch script
├── openjordy_api_server.py           # FastAPI proxy + PRM scoring + sample submission
├── openjordy_rollout.py              # Async rollout worker (bridges API server ↔ SLIME trainer)
