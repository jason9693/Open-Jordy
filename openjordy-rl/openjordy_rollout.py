...
from openjordy_api_server import OpenJordyAPIServer
...
class AsyncRolloutWorker:
    def __init__(self, args, data_buffer):
        ...
        self._server = OpenJordyAPIServer(args=args, output_queue=self.output_queue, submission_enabled=self._submission_enabled)
...
async def _drain_output_queue(args, worker: AsyncRolloutWorker) -> list[list[Sample]]:
    ...
    while len(data) < target_data_size:
        ...
        if time.time() - last_progress > 30:
            print(
                f"[OpenJordyWorker] waiting for API-produced samples: {len(data)}/{target_data_size}, "
    ...
    print(f"[OpenJordyWorker] drained {len(data)} groups in {time.time() - start:.2f}s", flush=True)
    return data


def generate_rollout_openjordy(args, rollout_id, data_buffer, evaluation=False):
    ...
    if eval_scores:
        ...
        print(
            f"[OpenJordyWorker] prm_eval_score={extra_metrics['rollout/prm_eval_score']:.4f} "
            f"(n={len(eval_scores)})",
            flush=True,
        )
