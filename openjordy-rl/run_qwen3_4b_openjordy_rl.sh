...
SAVE_CKPT=${SAVE_CKPT:-/absolute/path/to/OpenJordy-RL/ckpt/qwen3-4b-openjordy-rl}
...
ROLLOUT_ARGS=(
   ...
   --rollout-function-path openjordy_rollout.generate_rollout_openjordy
...
CUSTOM_ARGS=(
   --custom-generate-function-path openjordy_api_server.generate
   --custom-rm-path openjordy_api_server.reward_func
)
...
if [ "${USE_WANDB}" = "1" ] && [ -n "${WANDB_KEY_VALUE}" ]; then
  WANDB_ARGS=(
    ...
    --wandb-group qwen3-4b-openjordy-rl
