# OpenJordy ACP Bridge

This document describes how the OpenJordy ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`openjordy acp` exposes an ACP agent over stdio and forwards prompts to a running
OpenJordy Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `openjordy acp` and speaks ACP messages over stdio.
...
`openjordy acp` resolves the Gateway URL and auth from CLI flags or config:
