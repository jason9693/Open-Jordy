...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.openjordy", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: OpenJordyPeekabooBridgeServices?

    private static let legacySocketDirectoryNames = ["jordydbot", "jordydis", "moltbot"]

    private static var openjordySocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        return Self.makeSocketPath(for: "OpenJordy", in: base)
    }
    ...
    private func startIfNeeded() async {
        ...
        let services = OpenJordyPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.openjordySocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.openjordySocketPath, privacy: .public)")
    }
        ...
        do {
            ...
            if linkValues?.isSymbolicLink == true {
                ...
                let destinationURL = URL(fileURLWithPath: destination, relativeTo: linkURL.deletingLastPathComponent())
                    .standardizedFileURL
                if destinationURL.path == URL(fileURLWithPath: Self.openjordySocketPath).standardizedFileURL.path {
                    return
                }
            ...
            try fileManager.createSymbolicLink(atPath: legacyPath, withDestinationPath: Self.openjordySocketPath)
        } catch {
...
@MainActor
private final class OpenJordyPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.openjordy.peekaboo")
