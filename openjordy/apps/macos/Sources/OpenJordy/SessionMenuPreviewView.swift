...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.openjordy", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OpenJordySessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OpenJordySessionPreviewEntry,
    ...
    private static func cache(payload: OpenJordySessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OpenJordyChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [OpenJordyKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [OpenJordyKit.AnyCodable]) -> [OpenJordyChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(OpenJordyChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: OpenJordyChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OpenJordyChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OpenJordyChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OpenJordyChatMessage) -> String? {
