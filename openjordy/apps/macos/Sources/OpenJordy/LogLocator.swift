...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["OPENJORDY_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/openjordy")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("openjordy-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("openjordy-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/openjordy/ (rolling or stdout), or nil if none exist.
