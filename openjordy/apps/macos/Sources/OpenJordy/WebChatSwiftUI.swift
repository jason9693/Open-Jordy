...
private let webChatSwiftLogger = Logger(subsystem: "ai.openjordy", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: OpenJordyChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> OpenJordyChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(OpenJordyChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> OpenJordyChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                OpenJordyGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    OpenJordyChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    OpenJordyAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<OpenJordyChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any OpenJordyChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = OpenJordyChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: OpenJordyChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<OpenJordyChatView>) -> NSViewController
    {
