...
enum OpenJordyLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = OpenJordyOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = OpenJordyFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        OpenJordyLogging.bootstrapIfNeeded()
        let label = OpenJordyLogging.makeLabel(subsystem: subsystem, category: category)
...
struct OpenJordyOSLogHandler: AppLogLevelBackedHandler {
...
struct OpenJordyFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = OpenJordyLogging.parseLabel(self.label)
