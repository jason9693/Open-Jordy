...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.openjordy/openjordy.json. Prefer OPENJORDY_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.openjordy/openjordy.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.openjordy/openjordy.json. Restart the gateway to apply."
        }
