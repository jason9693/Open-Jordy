...
enum DebugActions {
    private static let verboseDefaultsKey = "openjordy.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "openjordy.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = OpenJordyPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "OpenJordy", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = OpenJordyPaths.configURL
        guard
