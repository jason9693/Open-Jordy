...
enum CommandResolver {
    private static let projectRootDefaultsKey = "openjordy.gatewayProjectRootPath"
    private static let helperName = "openjordy"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let openjordyEntry = root.appendingPathComponent("openjordy.mjs").path
        if FileManager().isReadableFile(atPath: openjordyEntry) { return openjordyEntry }
        let binEntry = root.appendingPathComponent("bin/openjordy.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/openjordy")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let openjordyPaths = self.openjordyManagedPaths(home: home)
        if !openjordyPaths.isEmpty {
            extras.insert(contentsOf: openjordyPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + openjordyPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func openjordyManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".openjordy"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func openjordyExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectOpenJordyExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("openjordy.mjs").path,
            root.appendingPathComponent("bin/openjordy.js").path,
        ]
    ...
    static func hasAnyOpenJordyInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.openjordyExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func openjordyNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let openjordyPath = self.projectOpenJordyExecutable(projectRoot: root) {
            return [openjordyPath, subcommand] + extraArgs
        }
        if let openjordyPath = self.openjordyExecutable(searchPaths: searchPaths) {
            return [openjordyPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "openjordy", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            openjordy entrypoint missing (looked for dist/index.js or openjordy.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real openjordy CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/openjordy"
        ...
        \(cliSection)
        \(projectSection)
        if command -v openjordy >/dev/null 2>&1; then
          CLI="$(command -v openjordy)"
          openjordy \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/openjordy.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/openjordy.mjs"
            node "$PRJ/openjordy.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/openjordy.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/openjordy.js"
            node "$PRJ/bin/openjordy.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent openjordy"
          pnpm --silent openjordy \(quotedArgs);
        else
          echo "openjordy CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? OpenJordyConfigFile.loadDict()
