...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.openjordy.mac`) even if the shipped
    ...
    {
        if environment["OPENJORDY_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "openjordy.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "openjordy.nixMode") { return true }
