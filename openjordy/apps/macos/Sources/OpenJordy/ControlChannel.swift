...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: OpenJordyProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.openjordy", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: OpenJordyKit.AnyCodable]()) {
                $0[$1.key] = OpenJordyKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: OpenJordyProtocol.AnyCodable?) -> [String: OpenJordyProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: OpenJordyProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: OpenJordyKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: OpenJordyProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("openjordy.control.heartbeat")
    static let controlAgentEvent = Notification.Name("openjordy.control.agent")
}
