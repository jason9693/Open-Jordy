...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.openjordy", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["OPENJORDY_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = OpenJordyConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.openjordyExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "openjordy CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["OPENJORDY_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "openjordy@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "openjordy@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "openjordy@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "openjordy@\(target)"])
            }

        statusHandler("Installing openjordy@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed openjordy@\(target)")
        } else {
