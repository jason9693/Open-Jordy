...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersOpenJordyBinary() throws {
        let defaults = self.makeLocalDefaults()

        let tmp = try makeTempDirForTests()
        CommandResolver.setProjectRoot(tmp.path)

        let openjordyPath = tmp.appendingPathComponent("node_modules/.bin/openjordy")
        try makeExecutableForTests(at: openjordyPath)

        let cmd = CommandResolver.openjordyCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([openjordyPath.path, "gateway"]))
    }
    ...
    @Test func prefersOpenJordyBinaryOverPnpm() throws {
        let defaults = self.makeLocalDefaults()
        ...
        let openjordyPath = binDir.appendingPathComponent("openjordy")
        ...
        #expect(cmd.prefix(2).elementsEqual([openjordyPath.path, "rpc"]))
    }

    @Test func usesOpenJordyBinaryWithoutNodeRuntime() throws {
        let defaults = self.makeLocalDefaults()
    ...
    @Test func fallsBackToPnpm() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "openjordy", "rpc"]))
    }
    ...
    @Test func pnpmKeepsExtraArgsAfterSubcommand() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "openjordy", "health", "--json"]))
    ...
    @Test func buildsSSHCommandForRemoteMode() {
        ...
        defaults.set("openjordy@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/openjordy", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "openjordy@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/openjordy'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("openjordy"))
    ...
    @Test func configRootLocalOverridesRemoteDefaults() throws {
        ...
        #expect(cmd.first == openjordyPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
