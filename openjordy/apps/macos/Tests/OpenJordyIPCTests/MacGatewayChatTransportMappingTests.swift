...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: OpenJordyProtocol.AnyCodable(["ok": OpenJordyProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: OpenJordyProtocol.AnyCodable(["ok": OpenJordyProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = OpenJordyProtocol.AnyCodable([
            "runId": OpenJordyProtocol.AnyCodable("run-1"),
            "sessionKey": OpenJordyProtocol.AnyCodable("main"),
            "state": OpenJordyProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: OpenJordyProtocol.AnyCodable(["a": OpenJordyProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
