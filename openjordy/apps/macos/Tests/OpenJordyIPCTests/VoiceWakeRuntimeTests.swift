...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["openjordy"]
        let text = "hey openjordy"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey openjordy do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["openjordy"], minPostTriggerGap: 0.3)
