...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: OpenJordyProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: OpenJordyProtocol.AnyCodable] = [
            "presence": .init([OpenJordyProtocol.AnyCodable(entry)]),
        ]
        let payload = OpenJordyProtocol.AnyCodable(payloadMap)
