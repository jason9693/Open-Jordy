...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.openjordy.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to OpenJordy", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to OpenJordy gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to OpenJordy.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "OpenJordyShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "OpenJordy is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "OpenJordy Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("openjordy-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
