...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__OPENJORDY_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<OpenJordyTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.openjordy", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = OpenJordyTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> OpenJordyTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return OpenJordyTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> OpenJordyTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> OpenJordyTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return OpenJordyTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> OpenJordyTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = OpenJordyTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: OpenJordyTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: OpenJordyAgentEventPayload.self
            ) else {
                continue
            }
