...
final class RemindersService: RemindersServicing {
    func list(params: OpenJordyRemindersListParams) async throws -> OpenJordyRemindersListPayload {
        ...
        let payload: [OpenJordyReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return OpenJordyReminderPayload(
        ...
        return OpenJordyRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: OpenJordyRemindersAddParams) async throws -> OpenJordyRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = OpenJordyReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return OpenJordyRemindersAddPayload(reminder: payload)
    }
