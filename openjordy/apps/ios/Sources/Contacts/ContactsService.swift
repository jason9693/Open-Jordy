...
final class ContactsService: ContactsServicing {
    ...
    func search(params: OpenJordyContactsSearchParams) async throws -> OpenJordyContactsSearchPayload {
        ...
        return OpenJordyContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: OpenJordyContactsAddParams) async throws -> OpenJordyContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return OpenJordyContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return OpenJordyContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> OpenJordyContactPayload {
        OpenJordyContactPayload(
