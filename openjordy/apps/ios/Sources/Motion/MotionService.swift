...
final class MotionService: MotionServicing {
    func activities(params: OpenJordyMotionActivityParams) async throws -> OpenJordyMotionActivityPayload {
        ...
        let mapped: [OpenJordyMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return OpenJordyMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: OpenJordyPedometerParams) async throws -> OpenJordyPedometerPayload {
        ...
        let payload: OpenJordyPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = OpenJordyPedometerPayload(
