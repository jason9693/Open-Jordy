...
@MainActor
final class OpenJordyAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.openjordy.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.openjordy.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.openjordy.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "openjordy.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "openjordy.watch.promptId"
    static let sessionKeyKey = "openjordy.watch.sessionKey"
    static let actionPrimaryIDKey = "openjordy.watch.action.primary.id"
    static let actionPrimaryLabelKey = "openjordy.watch.action.primary.label"
    static let actionSecondaryIDKey = "openjordy.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "openjordy.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "openjordy.watch.action.primary"
    static let actionSecondaryIdentifier = "openjordy.watch.action.secondary"
    static let actionIdentifierPrefix = "openjordy.watch.action."
    static let actionIDKeyPrefix = "openjordy.watch.action.id."
    static let actionLabelKeyPrefix = "openjordy.watch.action.label."
    static let categoryPrefix = "openjordy.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: OpenJordyWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> OpenJordyWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return OpenJordyWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "OpenJordy" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [OpenJordyWatchAction]) -> [UNNotificationAction] {
...
@main
struct OpenJordyApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(OpenJordyAppDelegate.self) private var appDelegate
...
extension OpenJordyApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("OpenJordy: installing uncaught exception handler")
