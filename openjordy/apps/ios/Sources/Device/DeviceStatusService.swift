...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> OpenJordyDeviceStatusPayload {
        ...
        return OpenJordyDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> OpenJordyDeviceInfoPayload {
        ...
        return OpenJordyDeviceInfoPayload(
    ...
    private func batteryStatus() -> OpenJordyBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: OpenJordyBatteryState = switch device.batteryState {
        ...
        return OpenJordyBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> OpenJordyThermalStatusPayload {
        let state: OpenJordyThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return OpenJordyThermalStatusPayload(state: state)
    }

    private func storageStatus() -> OpenJordyStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return OpenJordyStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
