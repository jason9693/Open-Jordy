...
struct IOSGatewayChatTransport: OpenJordyChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.openjordy", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> OpenJordyChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(OpenJordyChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> OpenJordyChatHistoryPayload {
        ...
        return try JSONDecoder().decode(OpenJordyChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [OpenJordyChatAttachmentPayload]) async throws -> OpenJordyChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [OpenJordyChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(OpenJordyChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(OpenJordyGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenJordyGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenJordyChatEventPayload.self)
                        {
