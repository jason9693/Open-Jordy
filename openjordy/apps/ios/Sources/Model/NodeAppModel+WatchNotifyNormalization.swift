...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: OpenJordyWatchNotifyParams) -> OpenJordyWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [OpenJordyWatchAction]?,
        kind: String?,
        promptId: String?) -> [OpenJordyWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> OpenJordyWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return OpenJordyWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                OpenJordyWatchAction(id: "approve", label: "Approve"),
                OpenJordyWatchAction(id: "decline", label: "Decline", style: "destructive"),
                OpenJordyWatchAction(id: "open_phone", label: "Open iPhone"),
                OpenJordyWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            OpenJordyWatchAction(id: "done", label: "Done"),
            OpenJordyWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            OpenJordyWatchAction(id: "open_phone", label: "Open iPhone"),
            OpenJordyWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: OpenJordyWatchRisk?,
        priority: OpenJordyNotificationPriority?) -> OpenJordyWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: OpenJordyNotificationPriority?,
        risk: OpenJordyWatchRisk?) -> OpenJordyNotificationPriority?
    {
