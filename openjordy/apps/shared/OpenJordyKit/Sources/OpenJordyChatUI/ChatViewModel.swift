...
private let chatUILogger = Logger(subsystem: "ai.openjordy", category: "OpenJordyChatUI")

@MainActor
@Observable
public final class OpenJordyChatViewModel {
    public private(set) var messages: [OpenJordyChatMessage] = []
    ...
    public var attachments: [OpenJordyPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [OpenJordyChatPendingToolCall] = []
    public private(set) var sessions: [OpenJordyChatSessionEntry] = []
    private let transport: any OpenJordyChatTransport
    ...
    private var pendingToolCallsById: [String: OpenJordyChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any OpenJordyChatTransport) {
    ...
    public var sessionChoices: [OpenJordyChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [OpenJordyChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: OpenJordyPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [OpenJordyChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: OpenJordyChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: OpenJordyChatMessage) -> OpenJordyChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> OpenJordyChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return OpenJordyChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: OpenJordyChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [OpenJordyChatMessage],
        incoming: [OpenJordyChatMessage]) -> [OpenJordyChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [OpenJordyChatMessage]) -> [OpenJordyChatMessage] {
        var result: [OpenJordyChatMessage] = []
    ...
    private static func dedupeKey(for message: OpenJordyChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [OpenJordyChatMessageContent] = [
            OpenJordyChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> OpenJordyChatAttachmentPayload in
            OpenJordyChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> OpenJordyChatSessionEntry {
        OpenJordyChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: OpenJordyChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: OpenJordyChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: OpenJordyAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = OpenJordyChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> OpenJordyPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
