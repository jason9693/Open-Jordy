...
import Foundation
import Testing
@testable import OpenJordyChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> OpenJordyChatHistoryPayload
{
    OpenJordyChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> OpenJordyChatSessionEntry {
...
private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [OpenJordyChatHistoryPayload],
    sessionsResponses: [OpenJordyChatSessionsListResponse] = []) async -> (TestChatTransport, OpenJordyChatViewModel)
{
    let transport = TestChatTransport(historyResponses: historyResponses, sessionsResponses: sessionsResponses)
    let vm = await MainActor.run { OpenJordyChatViewModel(sessionKey: sessionKey, transport: transport) }
    return (transport, vm)
}

private func loadAndWaitBootstrap(
    vm: OpenJordyChatViewModel,
...
private func sendUserMessage(_ vm: OpenJordyChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, OpenJordyChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [OpenJordyChatHistoryPayload]
    private let sessionsResponses: [OpenJordyChatSessionsListResponse]

    private let stream: AsyncStream<OpenJordyChatTransportEvent>
    private let continuation: AsyncStream<OpenJordyChatTransportEvent>.Continuation

    init(
        historyResponses: [OpenJordyChatHistoryPayload],
        sessionsResponses: [OpenJordyChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<OpenJordyChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> OpenJordyChatHistoryPayload {
        ...
        return self.historyResponses.last ?? OpenJordyChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [OpenJordyChatAttachmentPayload]) async throws -> OpenJordyChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return OpenJordyChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> OpenJordyChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? OpenJordyChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: OpenJordyChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = OpenJordyChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func stripsInboundMetadataFromHistoryMessages() async throws {
        let history = OpenJordyChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [
                AnyCodable([
...
"""]],
        ...
        let vm = await MainActor.run { OpenJordyChatViewModel(sessionKey: "main", transport: transport) }
