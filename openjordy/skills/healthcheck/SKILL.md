---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for OpenJordy deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, OpenJordy cron scheduling for periodic checks, or version status checks on a machine running OpenJordy (laptop, workstation, Pi, VPS).
---

# OpenJordy Host Hardening
...
- Never claim OpenJordy changes the host firewall, SSH, or OS updates; it does not.
...
5. OpenJordy gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if OpenJordy can access sensitive data.
...
### 2) Run OpenJordy security audits (read-only)

As part of the default read-only checks, run `openjordy security audit --deep`. Only offer alternatives if the user requests them:

1. `openjordy security audit` (faster, non-probing)
2. `openjordy security audit --json` (structured output)

Offer to apply OpenJordy safe defaults (numbered):

1. `openjordy security audit --fix`

Be explicit that `--fix` only tightens OpenJordy defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check OpenJordy version/update status (read-only)

As part of the default read-only checks, run `openjordy update status`.
...
- Credential hygiene notes (location of OpenJordy creds, prefer disk encryption)
...
- OpenJordy security audit (re-run)
...
After OpenJordy install or first hardening pass, run at least one baseline audit and version check:

- `openjordy security audit`
- `openjordy security audit --deep`
- `openjordy update status`

Ongoing monitoring is recommended. Use the OpenJordy cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `openjordy cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `openjordy update status`
...
Before creating, `openjordy cron list` and match on exact `name`. If found, `openjordy cron edit <id> ...`.
If not found, `openjordy cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `openjordy update status` (preferred for source checkouts and channels)
2. `npm view openjordy version` (published npm version)

## OpenJordy command accuracy

Use only supported commands and flags:

- `openjordy security audit [--deep] [--fix] [--json]`
- `openjordy status` / `openjordy status --deep`
- `openjordy health --json`
- `openjordy update status`
- `openjordy cron add|list|runs|run`

Do not invent CLI flags or imply OpenJordy enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by OpenJordy compaction:
