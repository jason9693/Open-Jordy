...
describe("shared/frontmatter", () => {
  ...
  test("resolveOpenJordyManifestBlock parses JSON5 metadata and picks openjordy block", () => {
    const frontmatter = {
      metadata: "{ openjordy: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveOpenJordyManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveOpenJordyManifestBlock returns undefined for invalid input", () => {
    expect(resolveOpenJordyManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveOpenJordyManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveOpenJordyManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
