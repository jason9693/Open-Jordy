...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "openjordy-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".openjordy", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<OpenJordyConfig> = {},
): OpenJordyConfig {
  const base: OpenJordyConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<OpenJordyConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<OpenJordyConfig>,
