...
export type RunCronAgentTurnResult = {
   ...
   * messages.  See: https://github.com/openjordy/openjordy/issues/15692
   */
  delivered?: boolean;
  /**
...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const isFastTestEnv = process.env.OPENJORDY_TEST_FAST === "1";
  ...
  // correct per-agent paths (e.g. ~/.openjordy/agents/<agentId>/agent/).
  ...
  const cfgWithAgentDefaults: OpenJordyConfig = {
    ...params.cfg,
    ...
    const fallbackResult = await runWithModelFallback({
      ...
      run: (providerOverride, modelOverride) => {
        ...
        if (isCliProvider(providerOverride, cfgWithAgentDefaults)) {
          ...
          // the configured timeoutSeconds. See: https://github.com/openjordy/openjordy/issues/29774
