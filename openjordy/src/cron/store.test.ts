...
const { makeStorePath } = createCronStoreHarness({ prefix: "openjordy-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses OPENJORDY_HOME for tilde expansion", () => {
    vi.stubEnv("OPENJORDY_HOME", "/srv/openjordy-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/openjordy-home", "cron", "jobs.json"));
  });
});
