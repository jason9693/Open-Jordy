...
function listAgents(cfg: OpenJordyConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: OpenJordyConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<OpenJordyConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: OpenJordyConfig): AgentLookupCache {
...
function pickFirstExistingAgentId(cfg: OpenJordyConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: OpenJordyConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
  byChannelAccountIndex: Map<string, EvaluatedBindingsIndex>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<OpenJordyConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  OpenJordyConfig,
  {
    bindingsRef: OpenJordyConfig["bindings"];
    agentsRef: OpenJordyConfig["agents"];
    sessionRef: OpenJordyConfig["session"];
...
function resolveRouteCacheForConfig(cfg: OpenJordyConfig): Map<string, ResolvedAgentRoute> {
