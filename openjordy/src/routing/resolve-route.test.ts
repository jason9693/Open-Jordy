...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: OpenJordyConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: OpenJordyConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/openjordy-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<OpenJordyConfig["bindings"]>[number];
