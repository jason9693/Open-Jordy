...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.OPENJORDY_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    OPENJORDY_PROFILE: profile,
    OPENJORDY_GATEWAY_PORT: String(port),
    OPENJORDY_GATEWAY_TOKEN: token,
    OPENJORDY_LAUNCHD_LABEL: resolvedLaunchdLabel,
    OPENJORDY_SYSTEMD_UNIT: systemdUnit,
    OPENJORDY_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    OPENJORDY_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.OPENJORDY_GATEWAY_TOKEN?.trim() || env.JORDYDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    OPENJORDY_GATEWAY_TOKEN: gatewayToken,
    OPENJORDY_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    OPENJORDY_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    OPENJORDY_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    OPENJORDY_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    OPENJORDY_LOG_PREFIX: "node",
    OPENJORDY_SERVICE_MARKER: NODE_SERVICE_MARKER,
    OPENJORDY_SERVICE_KIND: NODE_SERVICE_KIND,
    OPENJORDY_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  return {
    ...
    OPENJORDY_STATE_DIR: sharedEnv.stateDir,
    OPENJORDY_CONFIG_PATH: sharedEnv.configPath,
  };
}
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.OPENJORDY_STATE_DIR;
  const configPath = env.OPENJORDY_CONFIG_PATH;
