...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPENJORDY_GATEWAY_PORT).toBe("18789");
    expect(env.OPENJORDY_GATEWAY_TOKEN).toBe("secret");
    expect(env.OPENJORDY_SERVICE_MARKER).toBe("openjordy");
    expect(env.OPENJORDY_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OPENJORDY_SERVICE_VERSION).toBe("string");
    expect(env.OPENJORDY_SYSTEMD_UNIT).toBe("openjordy-gateway.service");
    if (process.platform === "darwin") {
      expect(env.OPENJORDY_LAUNCHD_LABEL).toBe("ai.openjordy.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OPENJORDY_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OPENJORDY_SYSTEMD_UNIT).toBe("openjordy-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.OPENJORDY_LAUNCHD_LABEL).toBe("ai.openjordy.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through OPENJORDY_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", OPENJORDY_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.OPENJORDY_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy JORDYDBOT_GATEWAY_TOKEN to OPENJORDY_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", JORDYDBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.OPENJORDY_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers OPENJORDY_GATEWAY_TOKEN over legacy JORDYDBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        OPENJORDY_GATEWAY_TOKEN: "openjordy-token",
        JORDYDBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.OPENJORDY_GATEWAY_TOKEN).toBe("openjordy-token");
  });

  it("omits OPENJORDY_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        OPENJORDY_GATEWAY_TOKEN: "   ",
        JORDYDBOT_GATEWAY_TOKEN: " ",
      },
    });
    expect(env.OPENJORDY_GATEWAY_TOKEN).toBeUndefined();
  });
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openjordy"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", OPENJORDY_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openjordy-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", OPENJORDY_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openjordy"));
  });

  it("uses OPENJORDY_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", OPENJORDY_STATE_DIR: "/var/lib/openjordy" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/openjordy"));
  });

  it("expands ~ in OPENJORDY_STATE_DIR", () => {
    const env = { HOME: "/Users/test", OPENJORDY_STATE_DIR: "~/openjordy-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/openjordy-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { OPENJORDY_STATE_DIR: "C:\\State\\openjordy" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\openjordy");
  });
});
