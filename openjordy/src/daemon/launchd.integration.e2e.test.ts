...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `openjordy-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      OPENJORDY_LAUNCHD_LABEL: `ai.openjordy.launchd-int-${testId}`,
      OPENJORDY_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
