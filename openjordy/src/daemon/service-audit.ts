...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.OPENJORDY_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service OPENJORDY_GATEWAY_TOKEN does not match gateway.auth.token in openjordy.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `openjordy gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
