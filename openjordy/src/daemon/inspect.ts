...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "openjordy" | "jordydbot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["openjordy", "jordydbot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["openjordy_service_marker"];
  const kindKeys = ["openjordy_service_kind"];
...
function isOpenJordyGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.openjordy.");
}

function isOpenJordyGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("openjordy-gateway")) {
    return false;
  }
  return contents.toLowerCase().includes("gateway");
}

function isOpenJordyGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) {
    return false;
  }
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("openjordy gateway");
}
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("jordydbot") || lower.includes("moltbot");
}
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    if (!marker) {
      ...
      results.push({
        ...
        scope: params.scope,
        marker: isLegacyLabel(label) ? "jordydbot" : "moltbot",
        legacy: true,
      });
      continue;
    }
    ...
    if (marker === "openjordy" && isOpenJordyGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "openjordy" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (marker === "openjordy" && isOpenJordyGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isOpenJordyGatewayTaskName(name)) {
        continue;
      }
