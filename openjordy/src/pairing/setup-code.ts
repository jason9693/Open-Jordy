...
function resolveAuth(cfg: OpenJordyConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    env.OPENJORDY_GATEWAY_TOKEN?.trim() ||
    env.JORDYDBOT_GATEWAY_TOKEN?.trim() ||
    cfg.gateway?.auth?.token?.trim();
  const password =
    env.OPENJORDY_GATEWAY_PASSWORD?.trim() ||
    env.JORDYDBOT_GATEWAY_PASSWORD?.trim() ||
...
async function resolveGatewayPasswordSecretRef(
  ...
  const hasPasswordEnvCandidate = Boolean(
    env.OPENJORDY_GATEWAY_PASSWORD?.trim() || env.JORDYDBOT_GATEWAY_PASSWORD?.trim(),
  );
  ...
  if (mode !== "password") {
    const hasTokenCandidate =
      Boolean(env.OPENJORDY_GATEWAY_TOKEN?.trim() || env.JORDYDBOT_GATEWAY_TOKEN?.trim()) ||
