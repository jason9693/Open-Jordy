...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        OPENJORDY_BASE_ENV: "base",
        OPENJORDY_TO_REMOVE: undefined,
      },
      env: {
        OPENJORDY_TEST_ENV: "ok",
      },
    });

    expect(resolved.OPENJORDY_BASE_ENV).toBe("base");
    expect(resolved.OPENJORDY_TEST_ENV).toBe("ok");
    expect(resolved.OPENJORDY_TO_REMOVE).toBeUndefined();
  });
