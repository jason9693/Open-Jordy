...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.OPENJORDY_TTS_PREFS;
      process.env.OPENJORDY_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      ...
      try {
        ...
        process.env.OPENJORDY_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: OpenJordyConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
