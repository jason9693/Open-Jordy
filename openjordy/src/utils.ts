...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".openjordy");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.OPENJORDY_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$OPENJORDY_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via OPENJORDY_STATE_DIR.
