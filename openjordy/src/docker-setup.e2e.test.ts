...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "openjordy-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    OPENJORDY_GATEWAY_TOKEN: "test-token",
    OPENJORDY_CONFIG_DIR: join(sandbox.rootDir, "config"),
    OPENJORDY_WORKSPACE_DIR: join(sandbox.rootDir, "openjordy"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      OPENJORDY_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      OPENJORDY_EXTRA_MOUNTS: undefined,
      OPENJORDY_HOME_VOLUME: "openjordy-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("OPENJORDY_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("OPENJORDY_EXTRA_MOUNTS=");
    expect(envFile).toContain("OPENJORDY_HOME_VOLUME=openjordy-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("openjordy-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("openjordy-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg OPENJORDY_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(log).toContain("run --rm openjordy-cli onboard --mode local --no-install-daemon");
    expect(log).toContain("run --rm openjordy-cli config set gateway.mode local");
    expect(log).toContain("run --rm openjordy-cli config set gateway.bind lan");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      OPENJORDY_CONFIG_DIR: configDir,
      OPENJORDY_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("reuses existing config token when OPENJORDY_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    await writeFile(
      join(configDir, "openjordy.json"),
      JSON.stringify({ gateway: { auth: { mode: "token", token: "config-token-123" } } }),
    );

    const result = runDockerSetup(activeSandbox, {
      OPENJORDY_GATEWAY_TOKEN: undefined,
      OPENJORDY_CONFIG_DIR: configDir,
      OPENJORDY_WORKSPACE_DIR: workspaceDir,
    });
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("OPENJORDY_GATEWAY_TOKEN=config-token-123");
  });

  it("treats OPENJORDY_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("OPENJORDY_SANDBOX=");

    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg OPENJORDY_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg OPENJORDY_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      OPENJORDY_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker openjordy-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        OPENJORDY_SANDBOX: "1",
        OPENJORDY_DOCKER_SOCKET: socketPath,
      ...
      const gatewayStarts = log
        ...
        .filter(
          (line) =>
            ...
            line.includes(" up -d") &&
            line.includes("openjordy-gateway"),
        );
      expect(gatewayStarts).toHaveLength(2);
      expect(log).toContain(
        "run --rm --no-deps openjordy-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate openjordy-gateway"));
  ...
  it("rejects injected multiline OPENJORDY_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      OPENJORDY_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("OPENJORDY_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid OPENJORDY_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      OPENJORDY_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid OPENJORDY_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      OPENJORDY_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("OPENJORDY_HOME_VOLUME must match");
  });
  ...
  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:openjordy-gateway"');
