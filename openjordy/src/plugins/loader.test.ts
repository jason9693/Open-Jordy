...
import { __testing, loadOpenJordyPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "openjordy-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.OPENJORDY_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "openjordy.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.OPENJORDY_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadOpenJordyPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          openjordy: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.OPENJORDY_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.OPENJORDY_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadOpenJordyPlugins>) {
...
function useNoBundledPlugins() {
  process.env.OPENJORDY_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadOpenJordyPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.OPENJORDY_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.OPENJORDY_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadOpenJordyPlugins", () => {
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadOpenJordyPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadOpenJordyPlugins(options);
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    withEnv({ OPENJORDY_STATE_DIR: stateDir, JORDYDBOT_STATE_DIR: undefined }, () => {
