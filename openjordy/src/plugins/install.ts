...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing openjordy.extensions; update the plugin package to include openjordy.extensions (for example ["./dist/index.js"]). See https://docs.openjordy.ai/help/troubleshooting#plugin-install-fails-with-missing-openjordy-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_OPENJORDY_EXTENSIONS: "missing_openjordy_extensions",
  EMPTY_OPENJORDY_EXTENSIONS: "empty_openjordy_extensions",
...
function ensureOpenJordyExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_OPENJORDY_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json openjordy.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_OPENJORDY_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureOpenJordyExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from openjordy.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-openjordy"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "openjordy security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "openjordy security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromArchive(
  ...
  return await withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "openjordy-plugin-",
    timeoutMs,
    logger,
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "openjordy-npm-pack-",
    spec,
    timeoutMs,
