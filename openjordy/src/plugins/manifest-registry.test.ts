...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `openjordy-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "openjordy.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "openjordy.plugin.json");
  const linkedManifest = path.join(rootDir, "openjordy.plugin.json");
