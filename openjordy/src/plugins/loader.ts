...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OpenJordyPluginDefinition;
  register?: OpenJordyPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as OpenJordyPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as OpenJordyPluginDefinition;
...
export function loadOpenJordyPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverOpenJordyPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "openjordy/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "openjordy/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: OpenJordyPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as OpenJordyPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
