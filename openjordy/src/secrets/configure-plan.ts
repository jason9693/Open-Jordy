...
export function buildConfigureCandidates(config: OpenJordyConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  ...
  authoredOpenJordyConfig?: OpenJordyConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredOpenJordyConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const openjordyCandidates = discoverConfigSecretTargets(params.config)
    ...
    .map((entry) => {
      ...
      return {
        ...
        label: entry.path,
        configFile: "openjordy.json" as const,
  ...
  return [...openjordyCandidates, ...authCandidates].toSorted((a, b) =>
...
export function collectConfigureProviderChanges(params: {
  original: OpenJordyConfig;
  next: OpenJordyConfig;
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "openjordy secrets configure",
