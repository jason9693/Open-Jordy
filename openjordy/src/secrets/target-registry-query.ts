...
const OPENJORDY_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of OPENJORDY_COMPILED_SECRET_TARGETS) {
...
const OPENJORDY_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries =
    allowedTargetIds === null
      ? OPENJORDY_COMPILED_SECRET_TARGETS
      : Array.from(allowedTargetIds).flatMap(
          (targetId) => OPENJORDY_TARGETS_BY_ID.get(targetId) ?? [],
        );
