...
This directory contains hooks that ship with OpenJordy. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.openjordy/openjordy.json`:
...
2. Restart gateway: `pkill -9 -f 'openjordy.*gateway' && pnpm openjordy gateway`
3. Enable the hook: `openjordy hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.openjordy.ai/automation/hooks
