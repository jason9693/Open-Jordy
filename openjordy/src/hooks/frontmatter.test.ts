...
`;
    ...
    expect(parsed.openjordy.emoji).toBe("💾");
    expect(parsed.openjordy.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.openjordy.emoji).toBe("📝");
    expect(parsed.openjordy.events).toEqual(["command"]);
    expect(parsed.openjordy.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.openjordy.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"openjordy": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"openjordy": {"events": ["test"]}}');
  });
...
describe("resolveOpenJordyMetadata", () => {
  it("extracts openjordy metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOpenJordyMetadata(frontmatter);
  ...
  it("returns undefined when openjordy key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        openjordy: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with OpenJordy" },
            { id: "npm", kind: "npm", package: "@openjordy/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@openjordy/hook");
  });
...
`;
    ...
    const openjordy = resolveOpenJordyMetadata(frontmatter);
    expect(openjordy).toBeDefined();
    expect(openjordy?.emoji).toBe("💾");
    expect(openjordy?.events).toEqual(["command:new", "command:reset"]);
    expect(openjordy?.requires?.config).toEqual(["workspace.dir"]);
    expect(openjordy?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const openjordy = resolveOpenJordyMetadata(frontmatter);
    expect(openjordy?.emoji).toBe("disk");
    expect(openjordy?.events).toEqual(["command:new"]);
  });
});
