...
function ensureNodePathEnv(): string {
  ensureOpenJordyCliOnPath({ pathEnv: process.env.PATH ?? "" });
...
export async function resolveNodeHostGatewayCredentials(params: {
  ...
  const token =
    normalizeSecretInputString(env.OPENJORDY_GATEWAY_TOKEN) ??
  ...
  const shouldResolveConfiguredPassword =
    !normalizeSecretInputString(env.OPENJORDY_GATEWAY_PASSWORD) &&
    !tokenCanWin &&
