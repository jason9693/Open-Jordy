...
import {
  ...
  normalizeStringList,
  parseOpenJordyManifestInstallBase,
  parseFrontmatterBool,
  resolveOpenJordyManifestBlock,
  resolveOpenJordyManifestInstall,
  resolveOpenJordyManifestOs,
  resolveOpenJordyManifestRequires,
} from "../../shared/frontmatter.js";
import type {
  OpenJordySkillMetadata,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseOpenJordyManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
...
export function resolveOpenJordyMetadata(
  frontmatter: ParsedSkillFrontmatter,
): OpenJordySkillMetadata | undefined {
  const metadataObj = resolveOpenJordyManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveOpenJordyManifestRequires(metadataObj);
  const install = resolveOpenJordyManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveOpenJordyManifestOs(metadataObj);
  return {
