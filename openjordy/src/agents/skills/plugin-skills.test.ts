    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "openjordy.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "openjordy.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "openjordy.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("openjordy-");
  const acpxRoot = await tempDirs.make("openjordy-acpx-plugin-");
  const helperRoot = await tempDirs.make("openjordy-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("openjordy-");
  const pluginRoot = await tempDirs.make("openjordy-plugin-");
  const outsideDir = await tempDirs.make("openjordy-outside-");
