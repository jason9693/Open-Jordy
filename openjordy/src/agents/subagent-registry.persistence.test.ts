...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["OPENJORDY_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-subagent-"));
  ...
  it("uses isolated temp state when OPENJORDY_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "openjordy-test-state"));
  });
});
