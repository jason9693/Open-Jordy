...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): OpenJordyConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as OpenJordyConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): OpenJordyConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as OpenJordyConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: OpenJordyConfig, accountId = "media") {
    await handleTelegramAction(
