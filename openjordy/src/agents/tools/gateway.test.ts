...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    openjordy: process.env.OPENJORDY_GATEWAY_TOKEN,
    jordydbot: process.env.JORDYDBOT_GATEWAY_TOKEN,
  };
  ...
  beforeEach(() => {
    ...
    delete process.env.JORDYDBOT_GATEWAY_TOKEN;
  });

  afterAll(() => {
    if (envSnapshot.openjordy === undefined) {
      delete process.env.OPENJORDY_GATEWAY_TOKEN;
    } else {
      process.env.OPENJORDY_GATEWAY_TOKEN = envSnapshot.openjordy;
    }
    if (envSnapshot.jordydbot === undefined) {
      delete process.env.JORDYDBOT_GATEWAY_TOKEN;
    } else {
      process.env.JORDYDBOT_GATEWAY_TOKEN = envSnapshot.jordydbot;
    }
  });
  ...
  it("uses OPENJORDY_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.OPENJORDY_GATEWAY_TOKEN = "local-env-token";
    process.env.JORDYDBOT_GATEWAY_TOKEN = "legacy-env-token";
    configState.value = {
      gateway: {
