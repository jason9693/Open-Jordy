...
import { resolvePreferredOpenJordyTmpDir } from "../infra/tmp-openjordy-dir.js";
...
async function withOutsideHardlinkInOpenJordyTmp<T>(
  params: {
    openJordyTmpDir: string;
  ...
  const hardlinkPath = path.join(params.openJordyTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.openJordyTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.openJordyTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.openJordyTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const openJordyTmpDir = resolvePreferredOpenJordyTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred OpenJordy tmp root",
      media: path.join(openJordyTmpDir, "image.png"),
      expected: path.join(openJordyTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred OpenJordy tmp root",
      media: pathToFileURL(path.join(openJordyTmpDir, "photo.png")).href,
      expected: path.join(openJordyTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred OpenJordy tmp root",
      media: path.join(openJordyTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(openJordyTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(openJordyTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside openjordy tmp root",
      media: path.join(os.tmpdir(), "outside-openjordy", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked OpenJordy tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(openJordyTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(openJordyTmpDir, { recursive: true });
      const symlinkPath = path.join(openJordyTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked OpenJordy tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOpenJordyTmp(
      {
        openJordyTmpDir,
  ...
  it("rejects symlinked OpenJordy tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOpenJordyTmp(
