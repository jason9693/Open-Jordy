...
describe("createOpenJordyCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-read-"));
    try {
      ...
      const contents = "Hello from openjordy read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createOpenJordyCodingTools({ sandbox });
