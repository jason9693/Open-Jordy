...
function requireGatewayTool(agentSessionKey?: string) {
  const tool = createOpenJordyTools({
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-test-"));

    try {
      await withEnvAsync(
        { OPENJORDY_STATE_DIR: stateDir, OPENJORDY_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: openjordy --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/openjordy" } }\n}\n';
