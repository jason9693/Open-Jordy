...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("openjordy-ws-", async (workspaceDir) => {
      await withTempDir("openjordy-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createOpenJordyCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("openjordy-ws-", async (workspaceDir) => {
      await withTempDir("openjordy-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("openjordy-ws-", async (workspaceDir) => {
      const cfg: OpenJordyConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createOpenJordyCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-openjordy.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("openjordy-sandbox-", async (sandboxDir) => {
      await withTempDir("openjordy-workspace-", async (workspaceDir) => {
        ...
        const tools = createOpenJordyCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
