...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.OPENJORDY_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".openjordy", `workspace-${profile}`);
  }
  return path.join(home, ".openjordy", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".openjordy";
