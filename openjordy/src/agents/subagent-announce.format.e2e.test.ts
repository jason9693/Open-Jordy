...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/openjordy/openjordy/issues/31298
    previousFastTestEnv = process.env.OPENJORDY_TEST_FAST;
  ...
  afterAll(() => {
    if (previousFastTestEnv === undefined) {
      delete process.env.OPENJORDY_TEST_FAST;
      return;
    }
    process.env.OPENJORDY_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // OPENJORDY_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("OpenJordy runtime context (internal):");
