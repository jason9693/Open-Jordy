...
test("exec sets OPENJORDY_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-openjordy-shell", {
    command: "node -e \"process.stdout.write(process.env.OPENJORDY_SHELL || '')\"",
    pty: true,
  });
