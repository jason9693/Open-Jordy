...
type ModelsConfig = NonNullable<OpenJordyConfig["models"]>;
...
export async function ensureOpenJordyModelsJson(
  config?: OpenJordyConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = config ?? loadConfig();
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveOpenJordyAgentDir();
