...
type CoreToolDefinition = {
  ...
  includeInOpenJordyGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const openjordyTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInOpenJordyGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:openjordy": openjordyTools,
