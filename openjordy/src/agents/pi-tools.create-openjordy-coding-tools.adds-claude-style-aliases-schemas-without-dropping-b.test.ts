...
const defaultTools = createOpenJordyCodingTools({ senderIsOwner: true });
...
describe("createOpenJordyCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: OpenJordyConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createOpenJordyCodingTools({
    ...
    const anthropicTools = createOpenJordyCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: OpenJordyConfig = {
      tools: {
        exec: {
    ...
    const allowed = createOpenJordyCodingTools({
    ...
    const denied = createOpenJordyCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createOpenJordyCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
