...
const GOOGLE_LIVE_SINGLE_KEY = "OPENJORDY_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "OPENJORDY_LIVE_ANTHROPIC_KEY",
    listVar: "OPENJORDY_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "OPENJORDY_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `OPENJORDY_LIVE_${base}_KEY`;
