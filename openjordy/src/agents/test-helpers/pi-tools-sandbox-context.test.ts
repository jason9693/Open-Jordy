...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("openjordy-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("openjordy-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("openjordy-sbx-");
