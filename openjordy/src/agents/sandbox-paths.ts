...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const openJordyTmpDir = path.resolve(resolvePreferredOpenJordyTmpDir());
  if (!isPathInside(openJordyTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: openJordyTmpDir });
  return resolved;
}
