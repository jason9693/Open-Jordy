 ...
 * A common mistake — caused by the similarity with the `openjordy.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (openjordy.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.OPENJORDY_AUTH_STORE_READONLY === "1";
