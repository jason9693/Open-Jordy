...
vi.mock("./models-config.js", async (importOriginal) => {
  ...
  return {
    ...mod,
    ensureOpenJordyModelsJson: vi.fn(async () => ({ wrote: false })),
  };
});
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): OpenJordyConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): OpenJordyConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): OpenJordyConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-workspace-"));
    const now = Date.now();
