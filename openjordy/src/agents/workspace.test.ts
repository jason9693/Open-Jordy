...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses OPENJORDY_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      OPENJORDY_HOME: "/srv/openjordy-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/openjordy-home"), ".openjordy", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".openjordy", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("openjordy-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-workspace-hardlink-"));
    try {
