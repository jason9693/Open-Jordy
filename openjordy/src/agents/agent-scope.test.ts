...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: OpenJordyConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/openjordy" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: OpenJordyConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: OpenJordyConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: OpenJordyConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: OpenJordyConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: OpenJordyConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: OpenJordyConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: OpenJordyConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
    ...
    const cfg: OpenJordyConfig = {
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/openjordy-restricted",
            tools: {
              allow: ["read"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/openjordy");
  });

  it("uses OPENJORDY_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "openjordy-home");
    vi.stubEnv("OPENJORDY_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as OpenJordyConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".openjordy", "workspace"));
  });

  it("uses OPENJORDY_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "openjordy-home");
    vi.stubEnv("OPENJORDY_HOME", home);
    // Clear state dir so it falls back to OPENJORDY_HOME
    vi.stubEnv("OPENJORDY_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as OpenJordyConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".openjordy", "agents", "main", "agent"));
  });
});
