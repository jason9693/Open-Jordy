...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "openjordy.sandbox=1");
  args.push("--label", `openjordy.sessionKey=${params.scopeKey}`);
  args.push("--label", `openjordy.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `openjordy.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "openjordy.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`openjordy sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`openjordy sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("openjordy sandbox recreate --all");
}
