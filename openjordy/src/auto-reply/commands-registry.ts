...
export function isCommandEnabled(cfg: OpenJordyConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OpenJordyConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OpenJordyConfig),
...
export function getCommandDetection(_cfg?: OpenJordyConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OpenJordyConfig) {
