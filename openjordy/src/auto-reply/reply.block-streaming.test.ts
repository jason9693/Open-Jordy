...
function createReplyConfig(home: string, streamMode?: "block"): OpenJordyConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("openjordy-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("OPENJORDY_TEST_FAST", "1");
