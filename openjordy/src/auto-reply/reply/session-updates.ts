...
export async function buildQueuedSystemPrompt(params: {
  ...
  const resolveSystemEventTimezone = (cfg: OpenJordyConfig) => {
  ...
  const formatSystemEventTimestamp = (ts: number, cfg: OpenJordyConfig) => {
...
export async function ensureSkillSnapshot(params: {
  ...
  if (process.env.OPENJORDY_TEST_FAST === "1") {
