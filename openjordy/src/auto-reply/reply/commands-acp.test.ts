...
function createDiscordParams(commandBody: string, cfg: OpenJordyConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: OpenJordyConfig = baseCfg) {
  const params = createDiscordParams(commandBody, cfg);
  params.ctx.MessageThreadId = defaultThreadId;
  return params;
}

async function runDiscordAcpCommand(commandBody: string, cfg: OpenJordyConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: OpenJordyConfig = baseCfg) {
...
describe("/acp command", () => {
  ...
  it("spawns an ACP session and binds a Discord thread", async () => {
    ...
    const result = await runDiscordAcpCommand("/acp spawn codex --cwd /home/bob/jordyd");
    ...
    expect(hoisted.sessionBindingBindMock).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        metadata: expect.objectContaining({
          introText: expect.stringContaining("cwd: /home/bob/jordyd"),
        }),
      }),
    );
