...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop openjordy!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@openjordy_bot", { botUsername: "openjordy_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@openjordy_bot", { botUsername: "openjordy_bot" })).toBe(true);
