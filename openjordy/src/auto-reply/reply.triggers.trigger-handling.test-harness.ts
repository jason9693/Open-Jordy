...
type TempHomeEnvSnapshot = {
  ...
  openjordyHome: string | undefined;
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    openjordyHome: process.env.OPENJORDY_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("OPENJORDY_HOME", snapshot.openjordyHome);
  restoreKey("OPENJORDY_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.OPENJORDY_STATE_DIR = join(home, ".openjordy");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "openjordy-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".openjordy", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): OpenJordyConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "openjordy"),
