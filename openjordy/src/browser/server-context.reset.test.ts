...
function localOpenJordyProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "openjordy",
...
function createLocalOpenJordyResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localOpenJordyProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveOpenJordyUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "openjordy-reset-"));
    const profileDir = path.join(tempRoot, "openjordy");
    ...
    const ops = createLocalOpenJordyResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveOpenJordyUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "openjordy-reset-no-own-"));
    const profileDir = path.join(tempRoot, "openjordy");
    fs.mkdirSync(profileDir, { recursive: true });
