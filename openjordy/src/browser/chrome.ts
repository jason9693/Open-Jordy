...
export function resolveOpenJordyUserDataDir(profileName = DEFAULT_OPENJORDY_BROWSER_PROFILE_NAME) {
...
export async function launchOpenJordyChrome(
  ...
  const userDataDir = resolveOpenJordyUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_OPENJORDY_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateOpenJordyProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 openjordy browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`openjordy browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`openjordy browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 openjordy browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopOpenJordyChrome(
