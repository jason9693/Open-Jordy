...
export function createProfileAvailability({
  ...
  const waitForCdpReadyAfterLaunch = async (): Promise<void> => {
    // launchOpenJordyChrome() can return before Chrome is fully ready to serve /json/version + CDP WS.
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (!httpReachable) {
      ...
      const launched = await launchOpenJordyChrome(current.resolved, profile);
      attachRunning(launched);
      try {
        await waitForCdpReadyAfterLaunch();
      } catch (err) {
        await stopOpenJordyChrome(launched).catch(() => {});
        setProfileRunning(null);
        throw err;
      }
      return;
    ...
    if (!profileState.running) {
      throw new Error(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by openjordy. ` +
          `Run action=reset-profile profile=${profile.name} to kill the process.`,
      );
    }

    await stopOpenJordyChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchOpenJordyChrome(current.resolved, profile);
