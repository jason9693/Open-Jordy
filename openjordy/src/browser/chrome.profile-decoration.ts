...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".openjordy-profile-decorated");
}
 ...
 */
export function decorateOpenJordyProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OPENJORDY_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OPENJORDY_BROWSER_COLOR).toUpperCase();
