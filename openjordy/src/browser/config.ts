import type { BrowserConfig, BrowserProfileConfig, OpenJordyConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_OPENJORDY_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_OPENJORDY_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "openjordy" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OPENJORDY_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OPENJORDY_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an OpenJordy browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for openjordy/openjordy CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OpenJordyConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OPENJORDY_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_OPENJORDY_BROWSER_PROFILE_NAME]
        ? DEFAULT_OPENJORDY_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "openjordy";
