...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("openjordy");
    expect(profile?.driver).toBe("openjordy");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const openjordy = resolveProfile(resolved, "openjordy");
    expect(openjordy?.driver).toBe("openjordy");
    expect(openjordy?.cdpPort).toBe(18800);
    expect(openjordy?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from OPENJORDY_GATEWAY_PORT when unset", () => {
    withEnv({ OPENJORDY_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const openjordy = resolveProfile(resolved, "openjordy");
      expect(openjordy?.cdpPort).toBe(19012);
      expect(openjordy?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ OPENJORDY_GATEWAY_PORT: undefined }, () => {
      ...
      const openjordy = resolveProfile(resolved, "openjordy");
      expect(openjordy?.cdpPort).toBe(19022);
      expect(openjordy?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(openjordy?.cdpPort).toBe(19000);
    expect(openjordy?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "openjordy");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        openjordy: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("openjordy");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to openjordy profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps openjordy default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("openjordy");
    ...
    it("keeps openjordy default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("openjordy");
    ...
    it("keeps openjordy default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
