...
const RELAY_AUTH_HEADER = "x-openjordy-relay-token";
  ...
  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/OpenJordy-Extension-Relay",
            revision: "0",
            userAgent: "OpenJordy-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    const server = createServer((req, res) => {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: "OpenJordy/extension-relay",
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedOpenJordyRelay({
