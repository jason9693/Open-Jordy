...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-openjordy-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-openjordy-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-openjordy-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the OpenJordy gateway (OpenJordy.app menubar, or \`${formatCliCommand("openjordy gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the OpenJordy browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the OpenJordy browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
