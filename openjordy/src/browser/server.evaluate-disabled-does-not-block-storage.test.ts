  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          openjordy: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.OPENJORDY_GATEWAY_PORT;
    process.env.OPENJORDY_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.OPENJORDY_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.OPENJORDY_GATEWAY_PASSWORD;
