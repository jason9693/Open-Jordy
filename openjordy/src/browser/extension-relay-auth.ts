...
const RELAY_TOKEN_CONTEXT = "openjordy-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const OPENJORDY_RELAY_BROWSER = "OpenJordy/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.OPENJORDY_GATEWAY_TOKEN?.trim() || process.env.JORDYDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or OPENJORDY_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedOpenJordyRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === OPENJORDY_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
