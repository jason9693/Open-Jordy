...
describe("agent components", () => {
  const createCfg = (): OpenJordyConfig => ({}) as OpenJordyConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): OpenJordyConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/openjordy-sessions-test.json");
  });
