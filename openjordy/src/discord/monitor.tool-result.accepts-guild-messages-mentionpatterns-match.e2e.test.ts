  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/openjordy",
      },
    },
    session: { store: "/tmp/openjordy-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bopenjordy\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "openjordy: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "OpenJordy" },
          },
        },
        eventPatch: {
