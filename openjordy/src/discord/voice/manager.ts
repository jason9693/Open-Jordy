...
function resolveVoiceTtsConfig(params: { cfg: OpenJordyConfig; override?: TtsConfig }): {
  cfg: OpenJordyConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredOpenJordyTmpDir(), "discord-voice-"));
