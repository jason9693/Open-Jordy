...
declare const __OPENJORDY_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "openjordy";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["OPENJORDY_VERSION"],
      env["OPENJORDY_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current OpenJordy version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __OPENJORDY_VERSION__ === "string" ? __OPENJORDY_VERSION__ : undefined,
  bundledVersion: process.env.OPENJORDY_BUNDLED_VERSION,
});
