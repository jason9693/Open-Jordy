...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["OPENJORDY_GATEWAY_TOKEN", "OPENJORDY_GATEWAY_PASSWORD"]);
  ...
  it("uses OPENJORDY_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ OPENJORDY_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers OPENJORDY_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ OPENJORDY_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
