...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "OpenJordy", ".openjordy");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/openjordy-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".openjordy");
  ...
  it("anchors cloud detection to OS homedir when OPENJORDY_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".openjordy");
    const originalOpenJordyHome = process.env.OPENJORDY_HOME;
    process.env.OPENJORDY_HOME = "/tmp/openjordy-home-override";
    ...
    try {
      ...
      if (originalOpenJordyHome === undefined) {
        delete process.env.OPENJORDY_HOME;
      } else {
        process.env.OPENJORDY_HOME = originalOpenJordyHome;
      }
    }
  });
