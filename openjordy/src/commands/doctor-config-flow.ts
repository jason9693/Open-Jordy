...
import { OpenJordySchema } from "../config/zod-schema.js";
...
function stripUnknownConfigKeys(config: OpenJordyConfig): {
  config: OpenJordyConfig;
  removed: string[];
} {
  const parsed = OpenJordySchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: OpenJordyConfig) {
...
export function collectMissingDefaultAccountBindingWarnings(cfg: OpenJordyConfig): string[] {
...
function scanTelegramAllowFromUsernameEntries(cfg: OpenJordyConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: OpenJordyConfig): Promise<{
  config: OpenJordyConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: OpenJordyConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: OpenJordyConfig): {
  config: OpenJordyConfig;
  changes: string[];
} {
...
function scanMutableAllowlistEntries(cfg: OpenJordyConfig): MutableAllowlistHit[] {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: OpenJordyConfig): {
  config: OpenJordyConfig;
  changes: string[];
} {
...
async function maybeRepairAllowlistPolicyAllowFrom(cfg: OpenJordyConfig): Promise<{
  config: OpenJordyConfig;
  changes: string[];
}> {
  const channels = cfg.channels;
 ...
 * file support.
 */
function detectEmptyAllowlistPolicy(cfg: OpenJordyConfig): string[] {
  const channels = cfg.channels;
  if (!channels || typeof channels !== "object") {
    return [];
  }
  ...
  const checkAccount = (
    ...
    if (dmPolicy === "allowlist" && !hasAllowFromEntries(effectiveAllowFrom)) {
      warnings.push(
        `- ${prefix}.dmPolicy is "allowlist" but allowFrom is empty — all DMs will be blocked. Add sender IDs to ${prefix}.allowFrom, or run "${formatCliCommand("openjordy doctor --fix")}" to auto-migrate from pairing store when entries exist.`,
      );
    }
...
function collectExecSafeBinScopes(cfg: OpenJordyConfig): ExecSafeBinScopeRef[] {
...
function scanExecSafeBinCoverage(cfg: OpenJordyConfig): ExecSafeBinCoverageHit[] {
...
function scanExecSafeBinTrustedDirHints(cfg: OpenJordyConfig): ExecSafeBinTrustedDirHintHit[] {
...
function maybeRepairExecSafeBinProfiles(cfg: OpenJordyConfig): {
  config: OpenJordyConfig;
  changes: string[];
  warnings: string[];
} {
...
function scanLegacyToolsBySenderKeys(cfg: OpenJordyConfig): LegacyToolsBySenderKeyHit[] {
  const hits: LegacyToolsBySenderKeyHit[] = [];
  collectLegacyToolsBySenderKeyHits(cfg, [], hits);
  return hits;
}

function maybeRepairLegacyToolsBySenderKeys(cfg: OpenJordyConfig): {
  config: OpenJordyConfig;
  changes: string[];
} {
  const next = structuredClone(cfg);
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".openjordy");
  const targetPath = path.join(targetDir, "openjordy.json");
  try {
  ...
  const legacyCandidates = [
    path.join(home, ".jordydbot", "jordydbot.json"),
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: OpenJordyConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("openjordy doctor --fix")}" to apply compatibility migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("openjordy doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (shouldRepair) {
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("openjordy doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("openjordy doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("openjordy doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (toolsBySenderHits.length > 0) {
      ...
      note(
        [
          ...
          `- Run "${formatCliCommand("openjordy doctor --fix")}" to migrate legacy keys to typed id: entries.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (safeBinCoverage.length > 0) {
      ...
      lines.push(
        `- Run "${formatCliCommand("openjordy doctor --fix")}" to scaffold missing custom safeBinProfiles entries.`,
      );
      note(lines.join("\n"), "Doctor warnings");
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "openjordy doctor --fix" to remove these keys.');
    }
  }
