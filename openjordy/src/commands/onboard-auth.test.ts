...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under OPENJORDY_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("openjordy-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-oauth-sync-"));
    ...
    process.env.OPENJORDY_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-oauth-nosync-"));
    process.env.OPENJORDY_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside OPENJORDY_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-oauth-external-"));
    process.env.OPENJORDY_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* OPENJORDY_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPENJORDY_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("openjordy-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
