...
export function requireOpenJordyAgentDir(): string {
  const agentDir = process.env.OPENJORDY_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPENJORDY_AGENT_DIR not set");
  }
  return agentDir;
}
