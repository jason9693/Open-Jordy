...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOpenJordyAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("openjordy models auth setup-token")}\` or \`${formatCliCommand("openjordy configure")}\`.`
          : `Run \`${formatCliCommand("openjordy configure")}\` or set an API key env var.`;
