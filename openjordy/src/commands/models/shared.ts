...
export async function loadValidConfigOrThrow(): Promise<OpenJordyConfig> {
...
export async function updateConfig(
  mutator: (cfg: OpenJordyConfig) => OpenJordyConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OpenJordyConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OpenJordyConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("openjordy agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
