...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenJordyAgentDir: vi.fn().mockReturnValue("/tmp/openjordy-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/openjordy-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/openjordy-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOpenJordyAgentDir: mocks.resolveOpenJordyAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/openjordy-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOpenJordyAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/openjordy-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/openjordy-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/openjordy-agent-custom");
