...
function resolveMode(cfg: OpenJordyConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("OpenJordy doctor");

  const root = await resolveOpenJordyPackageRoot({
  ...
  let cfg: OpenJordyConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("openjordy configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("openjordy config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("openjordy setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("openjordy doctor --fix")}" to apply changes.`);
  }
