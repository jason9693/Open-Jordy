...
function pruneAuthProfiles(
  cfg: OpenJordyConfig,
  profileIds: Set<string>,
): { next: OpenJordyConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("openjordy models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("openjordy models auth setup-token")} or ${formatCliCommand(
      "openjordy configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "openjordy models auth login --provider openai-codex",
    )} or ${formatCliCommand("openjordy configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("openjordy configure")}\` or \`${formatCliCommand("openjordy onboard")}\`.`;
}
