...
function resolveAgentName(cfg: OpenJordyConfig, agentId: string) {
...
function resolveAgentModel(cfg: OpenJordyConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OpenJordyConfig): AgentSummary[] {
