...
type EnvSnapshot = {
  HOME?: string;
  OPENJORDY_HOME?: string;
  OPENJORDY_STATE_DIR?: string;
  OPENJORDY_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    OPENJORDY_HOME: process.env.OPENJORDY_HOME,
    OPENJORDY_STATE_DIR: process.env.OPENJORDY_STATE_DIR,
    OPENJORDY_OAUTH_DIR: process.env.OPENJORDY_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: OpenJordyConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: OpenJordyConfig) {
...
async function runStateIntegrityText(cfg: OpenJordyConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "openjordy-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.OPENJORDY_HOME = tempHome;
    process.env.OPENJORDY_STATE_DIR = path.join(tempHome, ".openjordy");
    delete process.env.OPENJORDY_OAUTH_DIR;
    fs.mkdirSync(process.env.OPENJORDY_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when OPENJORDY_OAUTH_DIR is explicitly configured", async () => {
    process.env.OPENJORDY_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: OpenJordyConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints openjordy-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: OpenJordyConfig = {};
    ...
    expect(text).toMatch(/openjordy sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/openjordy sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /openjordy sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
