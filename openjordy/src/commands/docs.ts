...
const SEARCH_TOOL = "https://docs.openjordy.ai/mcp.SearchOpenJordy";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.openjordy.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('openjordy docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.openjordy.ai/");
      runtime.log(`Search: ${formatCliCommand('openjordy docs "your query"')}`);
    }
    return;
  }
