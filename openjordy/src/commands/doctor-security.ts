...
import type { OpenJordyConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: OpenJordyConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("openjordy security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.openjordy/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("openjordy approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.openjordy.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("openjordy configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("openjordy config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("openjordy doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("openjordy config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('openjordy config set session.dmScope "per-channel-peer"') +
