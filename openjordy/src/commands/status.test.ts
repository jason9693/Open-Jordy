...
beforeAll(() => {
  envSnapshot = captureEnv(["OPENJORDY_PROFILE"]);
  process.env.OPENJORDY_PROFILE = "isolated";
});
...
vi.mock("../infra/openjordy-root.js", () => ({
  resolveOpenJordyPackageRoot: vi.fn().mockResolvedValue("/tmp/openjordy"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/openjordy/pnpm-lock.yaml",
      markerPath: "/tmp/openjordy/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.openjordy.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.openjordy.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("openjordy status --all") ||
          line.includes("openjordy --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("OPENJORDY_GATEWAY_TOKEN", "abcd1234", async () => {
