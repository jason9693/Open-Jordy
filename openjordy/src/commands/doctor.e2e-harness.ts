...
export const resolveOpenJordyPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadOpenJordyPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/openjordy.json",
    createConfigIO,
...
beforeEach(() => {
  ...
  resolveOpenJordyPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OPENJORDY_STATE_DIR;
  originalUpdateInProgress = process.env.OPENJORDY_UPDATE_IN_PROGRESS;
  process.env.OPENJORDY_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openjordy-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OPENJORDY_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OPENJORDY_UPDATE_IN_PROGRESS;
  } else {
    process.env.OPENJORDY_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
