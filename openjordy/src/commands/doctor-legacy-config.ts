...
export function normalizeCompatibilityConfigValues(cfg: OpenJordyConfig): {
  config: OpenJordyConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: OpenJordyConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as OpenJordyConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as OpenJordyConfig["browser"],
    };
    changes.push(
