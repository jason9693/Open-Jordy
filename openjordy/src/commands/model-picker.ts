...
type PromptDefaultModelResult = { model?: string; config?: OpenJordyConfig };
...
function resolveConfiguredModelRaw(cfg: OpenJordyConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: OpenJordyConfig): string[] {
...
export function applyPrimaryModel(cfg: OpenJordyConfig, model: string): OpenJordyConfig {
...
export function applyModelAllowlist(cfg: OpenJordyConfig, models: string[]): OpenJordyConfig {
  const defaults = cfg.agents?.defaults;
