...
export async function statusCommand(
  ...
  runtime.log(theme.heading("OpenJordy status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`openjordy devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("openjordy devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("openjordy devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("openjordy security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("openjordy security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.openjordy.ai/faq");
  runtime.log("Troubleshooting: https://docs.openjordy.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("openjordy status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("openjordy logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("openjordy status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("openjordy gateway probe")}`);
  }
}
