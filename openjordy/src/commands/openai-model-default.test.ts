...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: OpenJordyConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: OpenJordyConfig) => { changed: boolean; next: OpenJordyConfig };
  defaultModel: string;
  overrideConfig: OpenJordyConfig;
  alreadyDefaultConfig: OpenJordyConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: OpenJordyConfig) => config,
      applyDefaultConfig: (config: OpenJordyConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: OpenJordyConfig = { agents: { defaults: {} } };
