...
function hasGatewayTokenCandidate(params: {
  ...
  const envToken =
    params.env.OPENJORDY_GATEWAY_TOKEN?.trim() || params.env.JORDYDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return true;
  }
  if (
...
function hasGatewayPasswordEnvCandidate(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.OPENJORDY_GATEWAY_PASSWORD?.trim() || env.JORDYDBOT_GATEWAY_PASSWORD?.trim());
}
