...
import { createOpenJordyTools } from "../agents/openjordy-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-openjordy-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-openjordy-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-openjordy-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-openjordy-thread-id")?.trim() || undefined;
  ...
  const allTools = createOpenJordyTools({
