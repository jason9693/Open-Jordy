...
export function resolveHooksConfig(cfg: OpenJordyConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: OpenJordyConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-openjordy-token"] === "string"
      ? req.headers["x-openjordy-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
