...
function cfg(input: Partial<OpenJordyConfig>): OpenJordyConfig {
  return input as OpenJordyConfig;
}

type ResolveFromConfigInput = Parameters<typeof resolveGatewayCredentialsFromConfig>[0];
type GatewayConfig = NonNullable<OpenJordyConfig["gateway"]>;
...
const DEFAULT_GATEWAY_ENV = {
  OPENJORDY_GATEWAY_TOKEN: "env-token",
  OPENJORDY_GATEWAY_PASSWORD: "env-password",
...
describe("resolveGatewayCredentialsFromConfig", () => {
  ...
  it("can disable legacy JORDYDBOT env fallback", () => {
    const resolved = resolveGatewayCredentialsFromConfig({
      cfg: cfg({
        gateway: {
          mode: "local",
      ...
      env: {
        ...
        JORDYDBOT_GATEWAY_PASSWORD: "legacy-password",
      } as NodeJS.ProcessEnv,
      includeLegacyEnv: false,
    });
