...
type GetReplyFromConfigFn = (
  ...
  configOverride?: OpenJordyConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `openjordy-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.OPENJORDY_CONFIG_PATH = path.join(root, "openjordy.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "openjordy.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "openjordy-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadOpenJordyPlugins: () => pluginRegistryState.registry,
  };
});
