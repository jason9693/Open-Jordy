...
function throwUnresolvedGatewaySecretInput(path: string): never {
  throw new Error(
    [
      ...
      "Fix: set OPENJORDY_GATEWAY_TOKEN/OPENJORDY_GATEWAY_PASSWORD, pass explicit --token/--password,",
...
function readGatewayTokenEnv(
  ...
  const primary = trimToUndefined(env.OPENJORDY_GATEWAY_TOKEN);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
  ...
  return trimToUndefined(env.JORDYDBOT_GATEWAY_TOKEN);
}
...
function readGatewayPasswordEnv(
  ...
  const primary = trimToUndefined(env.OPENJORDY_GATEWAY_PASSWORD);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
  ...
  return trimToUndefined(env.JORDYDBOT_GATEWAY_PASSWORD);
}
