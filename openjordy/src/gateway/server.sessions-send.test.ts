...
const { createOpenJordyTools } = await import("../agents/openjordy-tools.js");
...
type SessionSendTool = ReturnType<typeof createOpenJordyTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createOpenJordyTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["OPENJORDY_GATEWAY_PORT", "OPENJORDY_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.OPENJORDY_GATEWAY_PORT = String(gatewayPort);
  process.env.OPENJORDY_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("OPENJORDY_CONFIG_PATH missing in gateway test environment");
      }
