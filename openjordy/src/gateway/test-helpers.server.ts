...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "OPENJORDY_SKIP_BROWSER_CONTROL_SERVER",
  "OPENJORDY_SKIP_GMAIL_WATCHER",
  "OPENJORDY_SKIP_CANVAS_HOST",
  "OPENJORDY_BUNDLED_PLUGINS_DIR",
  "OPENJORDY_SKIP_CHANNELS",
  "OPENJORDY_SKIP_PROVIDERS",
  "OPENJORDY_SKIP_CRON",
  "OPENJORDY_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.OPENJORDY_STATE_DIR = path.join(tempHome, ".openjordy");
  delete process.env.OPENJORDY_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.OPENJORDY_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.OPENJORDY_SKIP_GMAIL_WATCHER = "1";
  process.env.OPENJORDY_SKIP_CANVAS_HOST = "1";
  process.env.OPENJORDY_SKIP_CHANNELS = "1";
  process.env.OPENJORDY_SKIP_PROVIDERS = "1";
  process.env.OPENJORDY_SKIP_CRON = "1";
  process.env.OPENJORDY_TEST_MINIMAL_GATEWAY = "1";
  process.env.OPENJORDY_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "openjordy-test-no-bundled-extensions")
    : "openjordy-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".openjordy-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".openjordy-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__openjordyTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__openjordyTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["OPENJORDY_GATEWAY_TOKEN"]);
  const prev = process.env.OPENJORDY_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.OPENJORDY_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.OPENJORDY_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
