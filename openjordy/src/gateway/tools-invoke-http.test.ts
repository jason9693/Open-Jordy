...
let lastCreateOpenJordyToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/openjordy-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateOpenJordyToolsContext?.agentTo,
          agentThreadId: lastCreateOpenJordyToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createOpenJordyTools: (ctx: Record<string, unknown>) => {
      lastCreateOpenJordyToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateOpenJordyToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-openjordy-message-to": "channel:24514",
        "x-openjordy-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
