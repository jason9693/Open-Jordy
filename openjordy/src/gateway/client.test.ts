...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("openjordy doctor --fix");
...
describe("GatewayClient security checks", () => {
  const envSnapshot = captureEnv(["OPENJORDY_ALLOW_INSECURE_PRIVATE_WS"]);
  ...
  it("allows ws:// to private addresses only with OPENJORDY_ALLOW_INSECURE_PRIVATE_WS=1", () => {
