...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.OPENJORDY_GATEWAY_URL) ??
      trimToUndefined(process.env.JORDYDBOT_GATEWAY_URL));
  ...
  const allowPrivateWs = process.env.OPENJORDY_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set OPENJORDY_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: openjordy doctor --fix",
...
function readGatewayTokenEnv(env: NodeJS.ProcessEnv): string | undefined {
  return trimToUndefined(env.OPENJORDY_GATEWAY_TOKEN) ?? trimToUndefined(env.JORDYDBOT_GATEWAY_TOKEN);
}

function readGatewayPasswordEnv(env: NodeJS.ProcessEnv): string | undefined {
  return (
    trimToUndefined(env.OPENJORDY_GATEWAY_PASSWORD) ?? trimToUndefined(env.JORDYDBOT_GATEWAY_PASSWORD)
  );
}
