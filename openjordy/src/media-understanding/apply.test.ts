...
const TEMP_MEDIA_PREFIX = "openjordy-media-";
...
function createGroqAudioConfig(): OpenJordyConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): OpenJordyConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): OpenJordyConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: OpenJordyConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredOpenJordyTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        OPENJORDY_AGENT_DIR: isolatedAgentDir,
