...
import type { ConfigFileSnapshot, OpenJordyConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: OpenJordyConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your OpenJordy state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your OpenJordy state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured =
    hasNonEmptyString(env.OPENJORDY_GATEWAY_TOKEN) || hasNonEmptyString(env.JORDYDBOT_GATEWAY_TOKEN);
  const envPasswordConfigured =
    hasNonEmptyString(env.OPENJORDY_GATEWAY_PASSWORD) ||
    hasNonEmptyString(env.JORDYDBOT_GATEWAY_PASSWORD);
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("openjordy security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.OPENJORDY_GATEWAY_TOKEN) ||
    hasNonEmptyString(env.JORDYDBOT_GATEWAY_TOKEN) ||
...
function collectLoggingFindings(cfg: OpenJordyConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: OpenJordyConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: OpenJordyConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("openjordy status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("openjordy security audit --deep")}".`,
    });
  }
