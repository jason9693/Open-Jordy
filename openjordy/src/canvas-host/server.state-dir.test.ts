...
describe("canvas host state dir defaults", () => {
  it("uses OPENJORDY_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("openjordy-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("OpenJordy Canvas");
      } finally {
        await handler.close();
      }
    });
