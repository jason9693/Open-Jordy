...
const createTempDir = () => tempDirs.make("openjordy-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets OPENJORDY_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "openjordy",
    });

    expect(env.OPENJORDY_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("openjordy");
  });

  it("overrides pre-existing OPENJORDY_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      OPENJORDY_SHELL: "wrong",
    });
    expect(env.OPENJORDY_SHELL).toBe("acp-client");
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "openjordy", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "openjordy", "dist", "entry.js");
    const shimPath = path.join(dir, "openjordy.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\openjordy\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/openjordy-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/openjordy-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/openjordy-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
