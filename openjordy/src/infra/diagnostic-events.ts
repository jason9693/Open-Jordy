...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __openjordyDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__openjordyDiagnosticEventsState) {
    globalStore.__openjordyDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__openjordyDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: OpenJordyConfig): boolean {
