...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.OPENJORDY_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for OpenJordy. You can keep running the web gateway without it: \`${formatCliCommand("openjordy gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
