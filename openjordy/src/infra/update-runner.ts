...
const DEFAULT_PACKAGE_NAME = "openjordy";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["openjordy", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "openjordy.mjs");
    ...
    const doctorStep = await runStep(
      step("openjordy doctor", doctorArgv, gitRoot, { OPENJORDY_UPDATE_IN_PROGRESS: "1" }),
    );
