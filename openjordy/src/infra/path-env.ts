...
function candidateBinDirs(opts: EnsureOpenJordyPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `openjordy` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "openjordy");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.OPENJORDY_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "openjordy"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `openjordy` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOpenJordyCliOnPath(opts: EnsureOpenJordyPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OPENJORDY_PATH_BOOTSTRAPPED)) {
    return;
  }
