...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.OPENJORDY_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "OpenJordy";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(OpenJordy\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.OPENJORDY_MDNS_HOSTNAME?.trim() ||
    process.env.JORDYDBOT_MDNS_HOSTNAME?.trim() ||
    "openjordy";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
