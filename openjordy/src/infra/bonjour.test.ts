...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.OPENJORDY_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    expect(gatewayCall?.[0]?.type).toBe("openjordy-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("openjordy (OpenJordy)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("openjordy");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("openjordy.local");
