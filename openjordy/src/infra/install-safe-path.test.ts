...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-install-safe-"));
    try {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-install-safe-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-install-safe-outside-"));
      try {
