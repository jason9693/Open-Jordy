...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): OpenJordyConfig {
  return {
...
describe("exec approval forwarder", () => {
  ...
  it("prefers turn-source routing over stale session last route", async () => {
    vi.useFakeTimers();
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "openjordy-exec-approval-forwarder-test-"));
    try {
