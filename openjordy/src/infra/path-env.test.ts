...
let ensureOpenJordyCliOnPath: typeof import("./path-env.js").ensureOpenJordyCliOnPath;

describe("ensureOpenJordyCliOnPath", () => {
  const envKeys = [
    "PATH",
    "OPENJORDY_PATH_BOOTSTRAPPED",
    "OPENJORDY_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureOpenJordyCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling openjordy exists", () => {
    const tmp = abs("/tmp/openjordy-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "openjordy");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.OPENJORDY_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.OPENJORDY_PATH_BOOTSTRAPPED = "1";
    ensureOpenJordyCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/openjordy-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "openjordy");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/openjordy-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "openjordy");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "openjordy");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/openjordy-path/case-linuxbrew");
