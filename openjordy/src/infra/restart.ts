...
export function triggerOpenJordyRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.OPENJORDY_SYSTEMD_UNIT,
        process.env.OPENJORDY_PROFILE,
      );
  ...
  const label =
    process.env.OPENJORDY_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPENJORDY_PROFILE);
