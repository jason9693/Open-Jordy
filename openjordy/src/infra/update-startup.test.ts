...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["OPENJORDY_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.OPENJORDY_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveOpenJordyPackageRoot } = await import("./openjordy-root.js"));
    ...
    vi.mocked(resolveOpenJordyPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveOpenJordyPackageRoot).mockResolvedValue("/opt/openjordy");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/openjordy/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
