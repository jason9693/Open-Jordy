...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__openjordy_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "openjordy-root");
...
describe("resolveOpenJordyPackageRoot", () => {
  let resolveOpenJordyPackageRoot: typeof import("./openjordy-root.js").resolveOpenJordyPackageRoot;
  let resolveOpenJordyPackageRootSync: typeof import("./openjordy-root.js").resolveOpenJordyPackageRootSync;

  beforeAll(async () => {
    ({ resolveOpenJordyPackageRoot, resolveOpenJordyPackageRootSync } =
      await import("./openjordy-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "openjordy");
    const pkgRoot = path.join(project, "node_modules", "openjordy");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "openjordy" }));

    expect(resolveOpenJordyPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "openjordy");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "openjordy.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "openjordy" }));

    expect(resolveOpenJordyPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveOpenJordyPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-openjordy package roots", async () => {
    const pkgRoot = fx("not-openjordy");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-openjordy" }));

    expect(resolveOpenJordyPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveOpenJordyPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveOpenJordyPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
