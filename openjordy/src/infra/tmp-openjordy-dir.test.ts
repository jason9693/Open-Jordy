...
import { POSIX_OPENJORDY_TMP_DIR, resolvePreferredOpenJordyTmpDir } from "./tmp-openjordy-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredOpenJordyTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `openjordy-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredOpenJordyTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_OPENJORDY_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredOpenJordyTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredOpenJordyTmpDir", () => {
  it("prefers /tmp/openjordy when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_OPENJORDY_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/openjordy when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_OPENJORDY_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/openjordy when /tmp/openjordy is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/openjordy when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/openjordy is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/openjordy is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/openjordy is group/other writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback OpenJordy temp dir/);
  });
