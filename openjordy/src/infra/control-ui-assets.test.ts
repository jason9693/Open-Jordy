...
vi.mock("./openjordy-root.js", () => ({
  resolveOpenJordyPackageRoot: vi.fn(async () => null),
  resolveOpenJordyPackageRootSync: vi.fn(() => null),
}));
...
let openjordyRoot: typeof import("./openjordy-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    openjordyRoot = await import("./openjordy-root.js");
  });
  ...
  it("uses resolveOpenJordyPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/openjordy");
    (
      openjordyRoot.resolveOpenJordyPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/openjordy"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "openjordy" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "openjordy.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-openjordy");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/openjordy-bundle");
    (
      openjordyRoot.resolveOpenJordyPackageRootSync as unknown as ReturnType<typeof vi.fn>
