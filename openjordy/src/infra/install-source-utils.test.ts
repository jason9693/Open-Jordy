...
const TEMP_DIR_PREFIX = "openjordy-install-source-utils-";
...
async function expectPackFallsBackToDetectedArchive(params: { stdout: string }) {
  const cwd = await createTempDir("openjordy-install-source-utils-");
  const archivePath = path.join(cwd, "openjordy-plugin-1.2.3.tgz");
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("openjordy-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
...
describe("resolveArchiveSourcePath", () => {
  it("returns not found error for missing archive paths", async () => {
    const result = await resolveArchiveSourcePath("/tmp/does-not-exist-openjordy-archive.tgz");
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "openjordy-plugin@1.2.3",
          name: "openjordy-plugin",
          version: "1.2.3",
          filename: "openjordy-plugin-1.2.3.tgz",
    ...
    const result = await runPack("openjordy-plugin@1.2.3", cwd);
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "openjordy-plugin",
        version: "1.2.3",
        resolvedSpec: "openjordy-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "openjordy-plugin@1.2.3", "--ignore-scripts", "--json"],
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "openjordy-plugin-1.2.3.tgz");
    await fs.writeFile(expectedArchivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: "npm notice created package\nopenjordy-plugin-1.2.3.tgz\n",
    });
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    mockPackCommandResult({
      stdout: "",
      stderr: "npm error code E404\nnpm error 404  '@openjordy/whatsapp@*' is not in this registry.",
      code: 1,
    });

    const result = await runPack("@openjordy/whatsapp", cwd);

    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("Package not found on npm");
      expect(result.error).toContain("@openjordy/whatsapp");
      expect(result.error).toContain("docs.openjordy.ai/tools/plugin");
    }
  });
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("openjordy-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "openjordy-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
        "npm notice creating package\n" +
        JSON.stringify([
          {
            id: "@openjordy/plugin-demo@2.0.0",
            filename: "openjordy-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@openjordy/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "openjordy-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@openjordy/plugin-demo@2.0.0",
      },
    });
  });
