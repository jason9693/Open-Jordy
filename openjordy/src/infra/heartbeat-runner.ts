...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: OpenJordyConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: OpenJordyConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: OpenJordyConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: OpenJordyConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: OpenJordyConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: OpenJordyConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: OpenJordyConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
