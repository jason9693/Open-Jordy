...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.OPENJORDY_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.OPENJORDY_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set OPENJORDY_APNS_TEAM_ID and OPENJORDY_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.OPENJORDY_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.OPENJORDY_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.OPENJORDY_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set OPENJORDY_APNS_PRIVATE_KEY_P8 or OPENJORDY_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading OPENJORDY_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createOpenJordyPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    openjordy: createOpenJordyPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
