...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OPENJORDY_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*openjordy-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "openjordy";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "openjordy");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "openjordy-gw");
  ...
  records.push(`_openjordy-gw._tcp IN PTR ${instanceLabel}._openjordy-gw._tcp`);
  records.push(`${instanceLabel}._openjordy-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._openjordy-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; openjordy-content-hash: ${contentHash}\n${contentBody}`;
}
