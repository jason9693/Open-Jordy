...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: OpenJordyConfig | null = null;
  ...
  get(key: string, cfg: OpenJordyConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: OpenJordyConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: OpenJordyConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: OpenJordyConfig): void {
