...
const mocks = vi.hoisted(() => ({
  ...
  loadOpenJordyPlugins: vi.fn(),
}));

const TEST_WORKSPACE_ROOT = "/tmp/openjordy-test-workspace";
...
vi.mock("../../plugins/loader.js", () => ({
  loadOpenJordyPlugins: mocks.loadOpenJordyPlugins,
}));
...
describe("resolveOutboundTarget channel resolution", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadOpenJordyPlugins.mockReset();
  });
  ...
  it("recovers telegram plugin resolution so announce delivery does not fail with Unsupported channel: telegram", () => {
    ...
    expect(mocks.loadOpenJordyPlugins).toHaveBeenCalledTimes(1);
  });
  ...
  it("retries bootstrap on subsequent resolve when the first bootstrap attempt fails", () => {
    ...
    expect(mocks.loadOpenJordyPlugins).toHaveBeenCalledTimes(2);
  });
});
