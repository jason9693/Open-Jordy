...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred OpenJordy tmp root", async () => {
    const tmpRoot = resolvePreferredOpenJordyTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideOpenJordy = path.join(os.tmpdir(), "outside-openjordy", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
