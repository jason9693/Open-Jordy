...
// Prefix for OpenJordy-generated action IDs to scope our handler
const OPENJORDY_ACTION_PREFIX = "openjordy:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${OPENJORDY_ACTION_PREFIX}`);

  // Handle OpenJordy modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
