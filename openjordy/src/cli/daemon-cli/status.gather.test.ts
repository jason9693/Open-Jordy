...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    OPENJORDY_STATE_DIR: "/tmp/openjordy-daemon",
    OPENJORDY_CONFIG_PATH: "/tmp/openjordy-daemon/openjordy.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.OPENJORDY_STATE_DIR ?? "/tmp/openjordy-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.OPENJORDY_CONFIG_PATH ?? `${stateDir}/openjordy.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/openjordy-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.OPENJORDY_STATE_DIR = "/tmp/openjordy-cli";
    process.env.OPENJORDY_CONFIG_PATH = "/tmp/openjordy-cli/openjordy.json";
