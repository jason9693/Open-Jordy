...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.OPENJORDY_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.OPENJORDY_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<OpenJordyConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live OpenJordy docs",
    hasSubcommands: false,
    register: async (program) => {
  ...
  },
  {
    ...
    hasSubcommands: true,
    register: async (program) => {
      const mod = await import("../jordybot-cli.js");
      mod.registerJordybotCli(program);
    },
  },
  {
    name: "pairing",
  ...
  {
    name: "plugins",
    description: "Manage OpenJordy plugins and extensions",
    hasSubcommands: true,
    register: async (program) => {
