...
${formatHelpExamples([
  ['openjordy agent --to +15555550123 --message "status update"', "Start a new session."],
  ['openjordy agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'openjordy agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'openjordy agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['openjordy agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.openjordy.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.openjordy.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['openjordy agents set-identity --agent main --name "OpenJordy" --emoji "🦞"', "Set name + emoji."],
  ["openjordy agents set-identity --agent main --avatar avatars/openjordy.png", "Set avatar path."],
  [
    "openjordy agents set-identity --workspace ~/.openjordy/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "openjordy agents set-identity --identity-file ~/.openjordy/workspace/IDENTITY.md --agent main",
