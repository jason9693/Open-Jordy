...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "openjordy", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "openjordy", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "openjordy", "status"]);
  });
