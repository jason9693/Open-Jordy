    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openjordy nodes status", "List known nodes with live status."],
          ["openjordy nodes pairing pending", "Show pending node pairing requests."],
          ['openjordy nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["openjordy nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.openjordy.ai/cli/nodes")}\n`,
    );
