...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@openjordy/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@openjordy/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@openjordy/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@openjordy/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @openjordy/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@openjordy/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@openjordy/plugin-alpha@1.2.3",
        installPath: "/tmp/openjordy/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@openjordy/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @openjordy/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@openjordy/plugin-alpha@latest",
      true,
      "/tmp/openjordy/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@openjordy/plugin-alpha@latest",
      installPath: "/tmp/openjordy/extensions/alpha",
      version: "1.2.3",
