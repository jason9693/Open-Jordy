...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.openjordy.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + OpenJordy hooks")
    ...
    .option("--hook-url <url>", "OpenJordy hook URL")
    .option("--hook-token <token>", "OpenJordy hook token")
