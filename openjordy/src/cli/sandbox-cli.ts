...
const SANDBOX_EXAMPLES = {
  main: [
    ["openjordy sandbox list", "List all sandbox containers."],
    ["openjordy sandbox list --browser", "List only browser containers."],
    ["openjordy sandbox recreate --all", "Recreate all containers."],
    ["openjordy sandbox recreate --session main", "Recreate a specific session."],
    ["openjordy sandbox recreate --agent mybot", "Recreate agent containers."],
    ["openjordy sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["openjordy sandbox list", "List all sandbox containers."],
    ["openjordy sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["openjordy sandbox recreate --all", "Recreate all containers."],
    ["openjordy sandbox recreate --session main", "Recreate a specific session."],
    ["openjordy sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["openjordy sandbox recreate --browser --all", "Recreate only browser containers."],
    ["openjordy sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["openjordy sandbox explain", "Show effective sandbox config."],
    ["openjordy sandbox explain --session agent:main:main", "Explain a specific session."],
    ["openjordy sandbox explain --agent work", "Explain an agent sandbox."],
    ["openjordy sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.openjordy.ai/cli/sandbox")}\n`,
    )
    .action(() => {
