    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openjordy memory status", "Show index and provider status."],
          ["openjordy memory index --force", "Force a full reindex."],
          ['openjordy memory search --query "deployment notes"', "Search indexed memory entries."],
          ["openjordy memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.openjordy.ai/cli/memory")}\n`,
    );
