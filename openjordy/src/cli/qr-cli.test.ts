...
describe("registerQrCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.stubEnv("OPENJORDY_GATEWAY_TOKEN", "");
    vi.stubEnv("JORDYDBOT_GATEWAY_TOKEN", "");
    vi.stubEnv("OPENJORDY_GATEWAY_PASSWORD", "");
    vi.stubEnv("JORDYDBOT_GATEWAY_PASSWORD", "");
  });
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("openjordy devices approve <requestId>");
  });
  ...
  it("uses OPENJORDY_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("OPENJORDY_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue({
      secrets: {
        providers: {
          default: { source: "env" },
