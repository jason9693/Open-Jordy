...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      OPENJORDY_GATEWAY_PORT: String(params.port),
      ...(params.token ? { OPENJORDY_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.OPENJORDY_STATE_DIR = "/tmp/openjordy-cli-state";
    process.env.OPENJORDY_CONFIG_PATH = "/tmp/openjordy-cli-state/openjordy.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ...
        OPENJORDY_STATE_DIR: "/tmp/openjordy-daemon-state",
        OPENJORDY_CONFIG_PATH: "/tmp/openjordy-daemon-state/openjordy.json",
        OPENJORDY_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.openjordy.gateway.plist",
    });
