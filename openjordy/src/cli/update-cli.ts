...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update OpenJordy and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["openjordy update", "Update a source checkout (git)"],
        ["openjordy update --channel beta", "Switch to beta channel (git + npm)"],
        ["openjordy update --channel dev", "Switch to dev channel (git + npm)"],
        ["openjordy update --tag beta", "One-off update to a dist-tag or version"],
        ["openjordy update --dry-run", "Preview actions without changing anything"],
        ["openjordy update --no-restart", "Update without restarting the service"],
        ["openjordy update --json", "Output result as JSON"],
        ["openjordy update --yes", "Non-interactive (accept downgrade prompts)"],
        ["openjordy update wizard", "Interactive update wizard"],
        ["openjordy --update", "Shorthand for openjordy update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run openjordy update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.openjordy.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openjordy update status", "Show channel + version status."],
          ["openjordy update status --json", "JSON output."],
          ["openjordy update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.openjordy.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
