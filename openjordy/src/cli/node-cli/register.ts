    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "openjordy node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["openjordy node status", "Check node host service status."],
          ["openjordy node install", "Install the node host service."],
          ["openjordy node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.openjordy.ai/cli/node")}\n`,
    );
