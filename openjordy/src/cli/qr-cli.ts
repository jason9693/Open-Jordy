...
function readGatewayTokenEnv(env: NodeJS.ProcessEnv): string | undefined {
  const primary = typeof env.OPENJORDY_GATEWAY_TOKEN === "string" ? env.OPENJORDY_GATEWAY_TOKEN : "";
  if (primary.trim().length > 0) {
    return primary.trim();
  }
  const legacy = typeof env.JORDYDBOT_GATEWAY_TOKEN === "string" ? env.JORDYDBOT_GATEWAY_TOKEN : "";
...
function readGatewayPasswordEnv(env: NodeJS.ProcessEnv): string | undefined {
  const primary =
    typeof env.OPENJORDY_GATEWAY_PASSWORD === "string" ? env.OPENJORDY_GATEWAY_PASSWORD : "";
  if (primary.trim().length > 0) {
    return primary.trim();
  }
  const legacy =
    typeof env.JORDYDBOT_GATEWAY_PASSWORD === "string" ? env.JORDYDBOT_GATEWAY_PASSWORD : "";
  if (legacy.trim().length > 0) {
    return legacy.trim();
  }
  return undefined;
...
export function registerQrCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.openjordy.ai/cli/qr")}\n`,
    )
    .option(
      "--remote",
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        const lines: string[] = [
          ...
          "Scan this with the OpenJordy iOS app (Onboarding -> Scan QR).",
          "",
        ];
        ...
        lines.push(
          ...
          `  ${theme.command("openjordy devices list")}`,
          `  ${theme.command("openjordy devices approve <requestId>")}`,
        );
