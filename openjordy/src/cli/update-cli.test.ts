...
import type { OpenJordyConfig, ConfigFileSnapshot } from "../config/types.openjordy.js";
...
vi.mock("../infra/openjordy-root.js", () => ({
  resolveOpenJordyPackageRoot: vi.fn(),
}));
...
const { resolveOpenJordyPackageRoot } = await import("../infra/openjordy-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/openjordy-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as OpenJordyConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/openjordy-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveOpenJordyPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("openjordy-update");
    setTty(false);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveOpenJordyPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("openjordy");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/openjordy-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "openjordy-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("OpenJordy update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as OpenJordyConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveOpenJordyPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("openjordy-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ OPENJORDY_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.OPENJORDY_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("openjordy-update-wizard");
    await withEnvAsync({ OPENJORDY_GIT_DIR: tempDir }, async () => {
      setTty(true);
