...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "openjordy-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "openjordy-bundled-skills-test-"));
    process.env.OPENJORDY_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
