...
function telegramCfg(): OpenJordyConfig {
  return { channels: { telegram: { botToken: "tok" } } } as OpenJordyConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: OpenJordyConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as OpenJordyConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as OpenJordyConfig;
...
function createSignalAccountOverrideCfg(): OpenJordyConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as OpenJordyConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as OpenJordyConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as OpenJordyConfig,
        accountId: undefined,
        params: {
