...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in OpenJordy config",
  ...
  const resolved = await promptSecretRefForOnboarding({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "OPENJORDY_SECRET",
      envVarFormatError:
        'Use an env var name like "OPENJORDY_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
