...
let originalOpenJordyLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.OPENJORDY_TEST_FILE_LOG;
  originalOpenJordyLogLevel = process.env.OPENJORDY_LOG_LEVEL;
  delete process.env.OPENJORDY_TEST_FILE_LOG;
  delete process.env.OPENJORDY_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.OPENJORDY_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalOpenJordyLogLevel === undefined) {
    delete process.env.OPENJORDY_LOG_LEVEL;
  } else {
    process.env.OPENJORDY_LOG_LEVEL = originalOpenJordyLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
