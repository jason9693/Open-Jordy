...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("openjordy-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "openjordy-sandbox-browser-global" },
      agentBrowser: { network: "openjordy-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("openjordy-sandbox-browser-agent");
  });
