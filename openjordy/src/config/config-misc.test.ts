...
import { buildWebSearchProviderConfig, withTempHome, writeOpenJordyConfig } from "./test-helpers.js";
import { OpenJordySchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = OpenJordySchema.safeParse({
      $schema: "https://openjordy.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://openjordy.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = OpenJordySchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = OpenJordySchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("flags legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      await writeOpenJordyConfig(home, {
        agents: { list: [{ id: "pi" }] },
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeOpenJordyConfig(home, {
        gateway: { bind: "${OPENJORDY_BIND}" },
      });

      const prev = process.env.OPENJORDY_BIND;
      process.env.OPENJORDY_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.OPENJORDY_BIND;
        } else {
          process.env.OPENJORDY_BIND = prev;
        }
      }
    });
  });
