...
function isWhatsAppConfigured(cfg: OpenJordyConfig): boolean {
...
function isGenericChannelConfigured(cfg: OpenJordyConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: OpenJordyConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: OpenJordyConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: OpenJordyConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: OpenJordyConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: OpenJordyConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: OpenJordyConfig, pluginId: string): OpenJordyConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
