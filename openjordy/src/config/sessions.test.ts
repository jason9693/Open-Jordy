...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ OPENJORDY_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-openjordy",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-openjordy#general");
  });
  ...
  it("derives session transcripts dir from OPENJORDY_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { OPENJORDY_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when OPENJORDY_STATE_DIR differs from stored paths", () => {
