...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "openjordy.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const envSnapshot = {
    ...
    OPENJORDY_PLUGIN_MANIFEST_CACHE_MS: process.env.OPENJORDY_PLUGIN_MANIFEST_CACHE_MS,
  };

  const validateInSuite = (raw: unknown) => validateConfigObjectWithPlugins(raw);

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-config-plugin-validation-"));
    ...
    process.env.OPENJORDY_STATE_DIR = path.join(suiteHome, ".openjordy");
    process.env.OPENJORDY_PLUGIN_MANIFEST_CACHE_MS = "10000";
  ...
  afterAll(async () => {
    ...
    clearPluginManifestRegistryCache();
    if (envSnapshot.OPENJORDY_STATE_DIR === undefined) {
      delete process.env.OPENJORDY_STATE_DIR;
    } else {
      process.env.OPENJORDY_STATE_DIR = envSnapshot.OPENJORDY_STATE_DIR;
    }
    if (envSnapshot.OPENJORDY_PLUGIN_MANIFEST_CACHE_MS === undefined) {
      delete process.env.OPENJORDY_PLUGIN_MANIFEST_CACHE_MS;
    } else {
      process.env.OPENJORDY_PLUGIN_MANIFEST_CACHE_MS = envSnapshot.OPENJORDY_PLUGIN_MANIFEST_CACHE_MS;
    }
  });
