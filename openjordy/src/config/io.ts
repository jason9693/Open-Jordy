...
import type { OpenJordyConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareOpenJordyVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  openjordy config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  openjordy config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: OpenJordyConfig,
  pathSegments: string[],
): { changed: boolean; next: OpenJordyConfig } {
...
function coerceConfig(value: unknown): OpenJordyConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OpenJordyConfig;
}
...
function stampConfigVersion(cfg: OpenJordyConfig): OpenJordyConfig {
...
function warnIfConfigFromFuture(cfg: OpenJordyConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareOpenJordyVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer OpenJordy (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as OpenJordyConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OpenJordyConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OpenJordyConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OpenJordyConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as OpenJordyConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.OPENJORDY_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.OPENJORDY_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.OPENJORDY_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.OPENJORDY_WATCH_SESSION === "string" &&
        deps.env.OPENJORDY_WATCH_SESSION.trim().length > 0
          ? deps.env.OPENJORDY_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.OPENJORDY_WATCH_COMMAND === "string" &&
        deps.env.OPENJORDY_WATCH_COMMAND.trim().length > 0
          ? deps.env.OPENJORDY_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `OPENJORDY_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: OpenJordyConfig | null = null;
let runtimeConfigSourceSnapshot: OpenJordyConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OPENJORDY_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OPENJORDY_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): OpenJordyConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): OpenJordyConfig {
