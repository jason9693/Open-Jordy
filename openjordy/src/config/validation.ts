...
import type { OpenJordyConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OpenJordyConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: OpenJordyConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: OpenJordyConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OpenJordySchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OpenJordyConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OpenJordyConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as OpenJordyConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as OpenJordyConfig,
  };
}
