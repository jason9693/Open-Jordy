...
import type { ConfigFileSnapshot } from "./types.openjordy.js";
import { OpenJordySchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(OpenJordySchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.openjordy/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/openjordy/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/openjordy/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/openjordy/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/openjordy/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = OpenJordySchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "OpenJordyConfig";
    const hints = mainSchemaHints;
