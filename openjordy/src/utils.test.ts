...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("openjordy-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("openjordy-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("openjordy-lid-a-", (first) => {
      withTempDirSync("openjordy-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.openjordy when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "openjordy-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers OPENJORDY_HOME over HOME", () => {
    vi.stubEnv("OPENJORDY_HOME", "/srv/openjordy-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/openjordy-home"));
...
describe("shortenHomePath", () => {
  it("uses $OPENJORDY_HOME prefix when OPENJORDY_HOME is set", () => {
    vi.stubEnv("OPENJORDY_HOME", "/srv/openjordy-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/openjordy-home")}/.openjordy/openjordy.json`)).toBe(
      "$OPENJORDY_HOME/.openjordy/openjordy.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $OPENJORDY_HOME replacement when OPENJORDY_HOME is set", () => {
    vi.stubEnv("OPENJORDY_HOME", "/srv/openjordy-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/openjordy-home")}/.openjordy/openjordy.json`),
    ).toBe("config: $OPENJORDY_HOME/.openjordy/openjordy.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/openjordy")).toBe(path.resolve(os.homedir(), "openjordy"));
  });
  ...
  it("prefers OPENJORDY_HOME for tilde expansion", () => {
    vi.stubEnv("OPENJORDY_HOME", "/srv/openjordy-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/openjordy")).toBe(path.resolve("/srv/openjordy-home", "openjordy"));
