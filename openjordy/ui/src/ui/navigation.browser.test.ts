...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/openjordy/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/openjordy");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/openjordy/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENJORDY_CONTROL_UI_BASE_PATH__ = "/openjordy";
    const app = mountApp("/openjordy/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/openjordy");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/openjordy/sessions");
  });
