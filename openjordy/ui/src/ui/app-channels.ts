...
export async function handleWhatsAppStart(host: OpenJordyApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: OpenJordyApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: OpenJordyApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: OpenJordyApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: OpenJordyApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: OpenJordyApp): string {
...
function resolveGatewayHttpAuthHeader(host: OpenJordyApp): string | null {
...
function buildGatewayHttpHeaders(host: OpenJordyApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: OpenJordyApp) {
...
export function handleNostrProfileToggleAdvanced(host: OpenJordyApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: OpenJordyApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: OpenJordyApp) {
  const state = host.nostrProfileFormState;
