...
The onboarding wizard is the **recommended** way to set up OpenJordy on macOS,
...
`openjordy dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
(`web_fetch` works without a key). Easiest path: `openjordy configure --section web`
...
2. **Workspace** — Location for agent files (default `~/.openjordy/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `openjordy doctor` first.
</Note>
...
Use `openjordy agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.openjordy/workspace-<agentId>`.
...
- CLI command reference: [`openjordy onboard`](/cli/onboard)
