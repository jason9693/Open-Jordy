---
summary: "Research notes: offline memory system for Jordyd workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.openjordy/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep OpenJordy integration
...
Target: Jordyd-style workspace (`agents.defaults.workspace`, default `~/.openjordy/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.openjordy/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Jordyd to see every session.
...
```
~/.openjordy/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in OpenJordy**, but keep a separable core library.

### Why integrate into OpenJordy?

- OpenJordy already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `openjordy memory recall "…" --k 25 --since 30d`
  - `openjordy memory reflect --since 7d`
...
Pragmatic take for `~/.openjordy/workspace`:
