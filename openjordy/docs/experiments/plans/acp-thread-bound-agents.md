...
This plan defines how OpenJordy should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- OpenJordy core owns ACP control plane concerns
...
OpenJordy should not reimplement ACP transport internals in core.
OpenJordy should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in OpenJordy, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under OpenJordy state dir
...
Core (must be in OpenJordy core):
...
- ACP event stream is normalized to OpenJordy reply chunks
...
For the first production backend (`extensions/acpx`), OpenJordy and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses OpenJordy `sessionKey`)
...
- OpenJordy consumes ndjson events from `acpx --format json --json-strict`
