...
`openjordy doctor` is the repair + migration tool for OpenJordy. It fixes stale
...
```bash
openjordy doctor --yes
```

Accept defaults without prompting (including restart/service/sandbox repair steps when applicable).

```bash
openjordy doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
openjordy doctor --repair --force
```
...
- Extra workspace dir detection (`~/openjordy`).
...
you to run `openjordy doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.openjordy/openjordy.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.openjordy/sessions/` to `~/.openjordy/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.openjordy/agent/` to `~/.openjordy/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.openjordy/credentials/*.json` (except `oauth.json`)
  - to `~/.openjordy/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `openjordy doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.openjordy` folders exist across
  home directories or when `OPENJORDY_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.openjordy/openjordy.json` is
...
offers to remove them and install the OpenJordy service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named OpenJordy gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `openjordy doctor --generate-gateway-token` to force token
...
- `openjordy doctor` prompts before rewriting supervisor config.
- `openjordy doctor --yes` accepts the default repair prompts.
- `openjordy doctor --repair` applies recommended fixes without prompts.
- `openjordy doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `openjordy gateway install --force`.
