...
read_when:
  - Running OpenJordy behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of OpenJordy
...
- You run OpenJordy behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. OpenJordy checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. OpenJordy extracts the user identity from the configured header
...
- OpenJordy can stay on loopback HTTP behind the proxy.
...
If OpenJordy itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://openjordy.example.com
    to: http://openjordy-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
openjordy.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy openjordy:18789 {
...
location / {
    ...
    proxy_pass http://openjordy:18789;
...
`openjordy security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update OpenJordy config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `openjordy security audit` and review findings
