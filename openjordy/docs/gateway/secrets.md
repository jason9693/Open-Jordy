...
OpenJordy supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
When onboarding runs in interactive mode and you choose SecretRef storage, OpenJordy runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). OpenJordy validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/OpenJordy QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/openjordy"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `openjordy.json` refs)
...
When reload-time activation fails after a healthy state, OpenJordy enters degraded secrets state.
...
Credential-sensitive command paths that opt in (for example `openjordy memory` remote-memory paths and `openjordy qr --remote`) can resolve supported SecretRefs via gateway snapshot RPC.
...
- Snapshot refresh after backend secret rotation is handled by `openjordy secrets reload`.
...
```bash
openjordy secrets audit --check
openjordy secrets configure
openjordy secrets audit --check
```
...
- plaintext values at rest (`openjordy.json`, `auth-profiles.json`, `.env`)
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `openjordy.json` refs)
...
- lets you select supported secret-bearing fields in `openjordy.json` plus `auth-profiles.json` for one agent scope
...
- `openjordy secrets configure --providers-only`
- `openjordy secrets configure --skip-provider-setup`
- `openjordy secrets configure --agent <id>`
...
```bash
openjordy secrets apply --from /tmp/openjordy-secrets-plan.json
openjordy secrets apply --from /tmp/openjordy-secrets-plan.json --dry-run
```
...
OpenJordy intentionally does not write rollback backups containing historical plaintext secret values.
