...
- During heartbeat runs, OpenJordy treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `openjordy sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, OpenJordy skips the heartbeat run entirely (no model call).
...
```bash
openjordy system event --text "Check for urgent follow-ups" --mode now
```
