...
- `openjordy gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `openjordy doctor` reports no blocking config/service issues.
- `openjordy channels status --probe` shows connected/ready channels.
...
openjordy config get agents.defaults.models
```
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `openjordy configure`). If you are running OpenJordy via Podman using the dedicated `openjordy` user, the config lives at `~openjordy/.openjordy/openjordy.json`.
...
openjordy system heartbeat last
openjordy logs --follow
```
...
openjordy approvals get --node <idOrNameOrIp>
openjordy logs --follow
openjordy status
```
...
openjordy browser start --browser-profile openjordy
...
```bash
openjordy gateway status
openjordy config get gateway.mode
openjordy config get gateway.remote.url
openjordy config get gateway.auth.mode
```
...
```bash
openjordy config get gateway.bind
openjordy config get gateway.auth.token
openjordy gateway status
openjordy logs --follow
```
