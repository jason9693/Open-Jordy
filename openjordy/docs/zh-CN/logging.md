...
你可以在 `~/.openjordy/openjordy.json` 中覆盖此设置：
...
所有日志配置都在 `~/.openjordy/openjordy.json` 的 `logging` 下。
...
- OpenJordy 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `openjordy plugins enable diagnostics-otel` 启用插件。
...
- `openjordy.tokens`（计数器，属性：`openjordy.token`、`openjordy.channel`、`openjordy.provider`、`openjordy.model`）
- `openjordy.cost.usd`（计数器，属性：`openjordy.channel`、`openjordy.provider`、`openjordy.model`）
- `openjordy.run.duration_ms`（直方图，属性：`openjordy.channel`、`openjordy.provider`、`openjordy.model`）
- `openjordy.context.tokens`（直方图，属性：`openjordy.context`、`openjordy.channel`、`openjordy.provider`、`openjordy.model`）

消息流：

- `openjordy.webhook.received`（计数器，属性：`openjordy.channel`、`openjordy.webhook`）
- `openjordy.webhook.error`（计数器，属性：`openjordy.channel`、`openjordy.webhook`）
- `openjordy.webhook.duration_ms`（直方图，属性：`openjordy.channel`、`openjordy.webhook`）
- `openjordy.message.queued`（计数器，属性：`openjordy.channel`、`openjordy.source`）
- `openjordy.message.processed`（计数器，属性：`openjordy.channel`、`openjordy.outcome`）
- `openjordy.message.duration_ms`（直方图，属性：`openjordy.channel`、`openjordy.outcome`）

队列 + 会话：

- `openjordy.queue.lane.enqueue`（计数器，属性：`openjordy.lane`）
- `openjordy.queue.lane.dequeue`（计数器，属性：`openjordy.lane`）
- `openjordy.queue.depth`（直方图，属性：`openjordy.lane` 或 `openjordy.channel=heartbeat`）
- `openjordy.queue.wait_ms`（直方图，属性：`openjordy.lane`）
- `openjordy.session.state`（计数器，属性：`openjordy.state`、`openjordy.reason`）
- `openjordy.session.stuck`（计数器，属性：`openjordy.state`）
- `openjordy.session.stuck_age_ms`（直方图，属性：`openjordy.state`）
- `openjordy.run.attempt`（计数器，属性：`openjordy.attempt`）
...
- `openjordy.webhook.error`
  - `openjordy.channel`、`openjordy.webhook`、`openjordy.chatId`、`openjordy.error`
- `openjordy.message.processed`
  - `openjordy.channel`、`openjordy.outcome`、`openjordy.chatId`、`openjordy.messageId`、`openjordy.sessionKey`、`openjordy.sessionId`、`openjordy.reason`
- `openjordy.session.stuck`
  - `openjordy.state`、`openjordy.ageMs`、`openjordy.queueDepth`、`openjordy.sessionKey`、`openjordy.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `openjordy doctor`。
