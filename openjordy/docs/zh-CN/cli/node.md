...
summary: "`openjordy node` 的 CLI 参考（无头节点主机）"
title: node
x-i18n:
...
```bash
openjordy node install --host <gateway-host> --port 18789
```
...
```bash
openjordy node status
openjordy node stop
openjordy node restart
openjordy node uninstall
```

使用 `openjordy node run` 运行前台节点主机（无服务）。
...
- `openjordy approvals --node <id|name|ip>`（从 Gateway 网关编辑）
