...
- `~/Library/Application Support/OpenJordy/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `openjordy-canvas://<session>/<path>`

示例：

- `openjordy-canvas://main/` → `<canvasRoot>/main/index.html`
- `openjordy-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `openjordy-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
openjordy nodes canvas present --node <id>
openjordy nodes canvas navigate --node <id> --url "/"
openjordy nodes canvas eval --node <id> --js "document.title"
openjordy nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__openjordy__/a2ui/
```
...
openjordy nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
openjordy nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "openjordy://agent?message=Review%20this%20design";
```
