---
read_when:
  - 在 DigitalOcean 上设置 OpenJordy
  - 寻找便宜的 VPS 托管来运行 OpenJordy
summary: 在 DigitalOcean 上运行 OpenJordy（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 OpenJordy

## 目标

以 **$6/月**（或使用预留定价 $4/月）在 DigitalOcean 上运行持久的 OpenJordy Gateway 网关。
...
- `~/.openjordy/workspace/` — 工作区（SOUL.md、记忆等）
