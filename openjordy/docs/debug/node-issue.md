...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OpenJordy
...
Running OpenJordy via Node with `tsx` fails at startup with:

```
[openjordy] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `openjordy status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch openjordy.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node openjordy.mjs status` works on Node 25.
