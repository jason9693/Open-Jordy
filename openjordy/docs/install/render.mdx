...
Deploy OpenJordy on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/openjordy/openjordy)
...
repository configures everything needed to run OpenJordy:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: OPENJORDY_STATE_DIR
        value: /data/.openjordy
      - key: OPENJORDY_WORKSPACE_DIR
        value: /data/workspace
      - key: OPENJORDY_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/openjordy`.
...
If you use the original OpenJordy repository, Render will not auto-deploy your OpenJordy. To update it, run a manual Blueprint sync from the dashboard.
...
For OpenJordy, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any OpenJordy host.
