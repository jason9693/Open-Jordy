...
Deploy OpenJordy on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/jordydbot-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/openjordy` — Control UI

## What you get

- Hosted OpenJordy Gateway + Control UI
...
- `OPENJORDY_STATE_DIR=/data/.openjordy` (recommended)
- `OPENJORDY_WORKSPACE_DIR=/data/workspace` (recommended)
- `OPENJORDY_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your OpenJordy state + workspace so you can migrate to another host without losing config or memory.
