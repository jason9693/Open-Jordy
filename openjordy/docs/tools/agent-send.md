---
summary: "Direct `openjordy agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `openjordy agent` (direct agent runs)

`openjordy agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `openjordy message --target`).
...
```bash
openjordy agent --to +15555550123 --message "status update"
...
openjordy agent --to +15555550123 --message "Summon reply" --deliver
