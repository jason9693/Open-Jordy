...
- **gateway host** → `openjordy` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.openjordy/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.openjordy/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `openjordy approvals allowlist ...`).
- `openjordy security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `openjordy doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.openjordy/exec-approvals.json` directly.

CLI: `openjordy approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
