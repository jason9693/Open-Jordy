...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `openjordy sandbox explain`.
...
Legacy `agent.*` configs are migrated by `openjordy doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=openjordy-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPENJORDY_STATE_DIR:-$HOME/.openjordy}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
