---
summary: "Chrome extension: let OpenJordy drive your existing Chrome tab"
read_when:
...
The OpenJordy Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate openjordy-managed Chrome profile.
...
OpenJordy then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the OpenJordy release (npm package) as static files. There is no separate “build” step.

After upgrading OpenJordy:

- Re-run `openjordy browser extension install` to refresh the installed files under your OpenJordy state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

OpenJordy ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `OPENJORDY_GATEWAY_TOKEN`)
...
- Open the tab you want OpenJordy to control.
...
- `ON`: attached; OpenJordy can drive that tab.
...
Debugging: `openjordy sandbox explain`
...
`openjordy browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `openjordy browser extension install` first to copy the extension to a stable location under your OpenJordy state directory.
...
- **This is not isolated** like the dedicated openjordy-managed profile.
