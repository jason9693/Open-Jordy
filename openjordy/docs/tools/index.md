---
summary: "Agent tool surface for OpenJordy (browser, canvas, nodes, message, cron) replacing legacy `openjordy-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `openjordy-*` skills
title: "Tools"
---

# Tools (OpenJordy)

OpenJordy exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `openjordy-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `openjordy.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, OpenJordy logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:openjordy`: all built-in OpenJordy tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`openjordy node run`).
...
OpenJordy tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `openjordy configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated OpenJordy-managed browser.
...
- Quick smoke: `openjordy nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `openjordy gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, OpenJordy uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- `sessions_spawn` supports inline file attachments for subagent runtime only (ACP rejects them). Each attachment has `name`, `content`, and optional `encoding` (`utf8` or `base64`) and `mimeType`. Files are materialized into the child workspace at `.openjordy/attachments/<uuid>/` with a `.manifest.json` metadata file. The tool returns a receipt with `count`, `totalBytes`, per file `sha256`, and `relDir`. Attachment content is automatically redacted from transcript persistence.
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, OpenJordy clamps `tools.sessions.visibility` to `tree`.
