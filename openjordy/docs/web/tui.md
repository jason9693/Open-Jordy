...
```bash
openjordy tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `openjordy tui --deliver`
...
- Local shell commands receive `OPENJORDY_SHELL=tui-local` in their environment.
...
- Check the Gateway logs: `openjordy logs --follow`.
- Confirm the agent can run: `openjordy status` and `openjordy models status`.
...
- No agents in picker: check `openjordy agents list` and your routing config.
