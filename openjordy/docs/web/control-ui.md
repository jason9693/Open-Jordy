...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/openjordy`)
...
If the page fails to load, start the Gateway first: `openjordy gateway`.
...
you revoke it with `openjordy devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.openjordy/openjordy.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop openjordy`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. OpenJordy
...
```bash
openjordy gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
OpenJordy **blocks** Control UI connections without device identity.
...
```bash
OPENJORDY_CONTROL_UI_BASE_PATH=/openjordy/ pnpm ui:build
```
