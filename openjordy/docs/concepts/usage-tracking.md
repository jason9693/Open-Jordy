...
- `/usage cost` in chats: local cost summary aggregated from OpenJordy session logs.
- CLI: `openjordy status --usage` prints a full per-provider breakdown.
- CLI: `openjordy channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
