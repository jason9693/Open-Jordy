...
- Same path: run the app from a fixed location (for OpenJordy, `dist/OpenJordy.app`).
...
```bash
sudo tccutil reset Accessibility ai.openjordy.mac
sudo tccutil reset ScreenCapture ai.openjordy.mac
...
Workaround: move files into the OpenJordy workspace (`~/.openjordy/workspace`) if you want to avoid per-folder grants.
