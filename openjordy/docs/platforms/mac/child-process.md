...
service via the external `openjordy` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.openjordy.gateway`
  (or `ai.openjordy.<profile>` when using `--profile`/`OPENJORDY_PROFILE`; legacy `com.openjordy.*` is supported).
