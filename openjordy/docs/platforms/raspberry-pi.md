---
summary: "OpenJordy on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up OpenJordy on a Raspberry Pi
  - Running OpenJordy on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# OpenJordy on Raspberry Pi

## Goal

Run a persistent, always-on OpenJordy Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
openjordy config set gateway.bind tailnet
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/openjordy-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
export NODE_COMPILE_CACHE=/var/tmp/openjordy-compile-cache
mkdir -p /var/tmp/openjordy-compile-cache
export OPENJORDY_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `OPENJORDY_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running OpenJordy, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit openjordy
```

```ini
[Service]
Environment=OPENJORDY_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/openjordy-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep OpenJordy state/cache on SSD-backed storage to avoid SD-card
...
Most OpenJordy features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled openjordy

# Enable if not
sudo systemctl enable openjordy

# Start on boot
sudo systemctl start openjordy
```
...
```bash
# Check logs
journalctl -u openjordy --no-pager -n 100

# Common fix: rebuild
cd ~/openjordy  # if using hackable install
npm run build
sudo systemctl restart openjordy
```
