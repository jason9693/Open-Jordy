---
summary: "Use OpenAI via API keys or Codex subscription in OpenJordy"
read_when:
  - You want to use OpenAI models in OpenJordy
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like OpenJordy.
...
```bash
openjordy onboard --auth-choice openai-api-key
# or non-interactive
openjordy onboard --openai-api-key "$OPENAI_API_KEY"
```
...
openjordy onboard --auth-choice openai-codex

# Or run OAuth directly
openjordy models auth login --provider openai-codex
```
...
OpenJordy uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), OpenJordy also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. OpenJordy enables it by default for
...
`baseUrl` on `api.openai.com`), OpenJordy now auto-enables OpenAI server-side
