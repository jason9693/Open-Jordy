---
summary: "Use MiniMax M2.5 in OpenJordy"
read_when:
  - You want MiniMax models in OpenJordy
...
- **Compatibility:** OpenJordy still accepts legacy `MiniMax-M2.5-Lightning` configs, but prefer
...
```bash
openjordy plugins enable minimax-portal-auth  # skip if already loaded.
openjordy gateway restart  # restart if gateway is already running
openjordy onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/openjordy/openjordy/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `openjordy.json`:
...
- Use `openjordy models list` and `openjordy models set minimax/MiniMax-M2.5` to switch.
...
- Running `openjordy configure` and selecting **MiniMax M2.5**, or
