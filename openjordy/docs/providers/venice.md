---
summary: "Use Venice AI privacy-focused models in OpenJordy"
read_when:
  - You want privacy-focused inference in OpenJordy
...
```bash
openjordy onboard --auth-choice venice-api-key
```
...
```bash
openjordy agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, OpenJordy shows all available Venice models. Pick based on your needs:
...
```bash
openjordy models set venice/claude-opus-45
openjordy models set venice/llama-3.3-70b
```

List all available models:

```bash
openjordy models list | grep venice
```

## Configure via `openjordy configure`

1. Run `openjordy configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
OpenJordy automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
openjordy agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
openjordy agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
openjordy agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
openjordy agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
openjordy agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `openjordy models list` to see currently available models. Some models may be temporarily offline.
