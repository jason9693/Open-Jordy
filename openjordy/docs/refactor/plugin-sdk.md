...
- Publish as `openjordy/plugin-sdk` (or export from core under `openjordy/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `OpenJordyPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OpenJordyConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OpenJordyConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OpenJordyConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OpenJordyConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OpenJordyConfig): string;
  };
};
```
...
- Introduce `openjordy/plugin-sdk`.
- Add `api.runtime` to `OpenJordyPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `openjordyRuntime: ">=2026.2.0"`).
