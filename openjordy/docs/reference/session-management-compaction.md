...
This document explains how OpenJordy manages sessions end-to-end:
...
OpenJordy is designed around a single **Gateway process** that owns session state.
...
OpenJordy persists sessions in two layers:
...
- Transcripts: `~/.openjordy/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

OpenJordy resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, OpenJordy reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.openjordy/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
OpenJordy intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (OpenJordy consumes the events, but Pi decides when to compact).
...
OpenJordy also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, OpenJordy bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, OpenJordy leaves it alone.
...
- `openjordy status` (CLI)
- `openjordy sessions` / `sessions --json`
...
OpenJordy supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- OpenJordy strips/suppresses this in the delivery layer.

As of `2026.1.10`, OpenJordy also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
OpenJordy uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but OpenJordy’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `openjordy status`.
