...
OpenJordy integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- OpenJordy owns lifecycle when `channels.signal.autoStart=true`.
...
- OpenJordy spawns `imsg rpc` as a child process (legacy iMessage integration).
