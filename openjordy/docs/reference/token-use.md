---
summary: "How OpenJordy builds prompt context and reports token usage + costs"
read_when:
...
OpenJordy tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

OpenJordy assembles its own system prompt on every run. It includes:
...
For images, OpenJordy downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from OpenJordy session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `openjordy status --usage` and `openjordy channels list` show
...
`cacheWrite`. If pricing is missing, OpenJordy shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. OpenJordy can
...
Anthropic's 1M context window is currently beta-gated. OpenJordy can inject the
...
OpenJordy skips the `context-1m-*` beta header because Anthropic currently
