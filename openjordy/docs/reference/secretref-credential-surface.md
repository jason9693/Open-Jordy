...
- In scope: strictly user-supplied credentials that OpenJordy does not mint or rotate.
- Out of scope: runtime-minted or rotating credentials, OAuth refresh material, and session-like artifacts.

## Supported credentials

### `openjordy.json` targets (`secrets configure` + `secrets apply` + `secrets audit`)
