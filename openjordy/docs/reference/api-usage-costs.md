...
OpenJordy features that can generate provider usage or paid API calls.
...
- `openjordy status --usage` and `openjordy channels list` show provider **usage windows**
...
OpenJordy can pick up credentials from:
...
`openjordy models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
