...
- With Anthropic API-key auth profiles, OpenJordy seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, OpenJordy injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
OpenJordy exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.openjordy/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$OPENJORDY_STATE_DIR/logs/cache-trace.jsonl`
...
- `OPENJORDY_CACHE_TRACE=1` enables cache tracing.
- `OPENJORDY_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `OPENJORDY_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `OPENJORDY_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `OPENJORDY_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
