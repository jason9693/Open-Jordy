...
Tlon is a decentralized messenger built on Urbit. OpenJordy connects to your Urbit ship and can
...
```bash
openjordy plugins install @openjordy/tlon
```

Local checkout (when running from a git repo):

```bash
openjordy plugins install ./extensions/tlon
```
...
By default, OpenJordy blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `openjordy message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OpenJordy replies in-thread.
