---
summary: "Synology Chat webhook setup and OpenJordy config"
read_when:
  - Setting up Synology Chat with OpenJordy
...
```bash
openjordy plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your OpenJordy gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in OpenJordy.
...
- Pairing approvals work with:
  - `openjordy pairing list synology-chat`
  - `openjordy pairing approve synology-chat <CODE>`
...
```bash
openjordy message send --channel synology-chat --target 123456 --text "Hello from OpenJordy"
openjordy message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
