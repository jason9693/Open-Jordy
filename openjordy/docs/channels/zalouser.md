...
read_when:
  - Setting up Zalo Personal for OpenJordy
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside OpenJordy.
...
- Install via CLI: `openjordy plugins install @openjordy/zalouser`
- Or from a source checkout: `openjordy plugins install ./extensions/zalouser`
...
- `openjordy pairing list zalouser`
- `openjordy pairing approve zalouser <code>`
...
- On startup, OpenJordy resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
Accounts map to `zalouser` profiles in OpenJordy state. Example:
...
- OpenJordy sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, OpenJordy sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `openjordy channels logout --channel zalouser && openjordy channels login --channel zalouser`
...
- The channel now runs fully in OpenJordy without external CLI binaries.
