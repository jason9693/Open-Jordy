...
Matrix is an open, decentralized messaging protocol. OpenJordy connects as a Matrix **user**
...
```bash
openjordy plugins install @openjordy/matrix
```

Local checkout (when running from a git repo):

```bash
openjordy plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `openjordy plugins install @openjordy/matrix`
   - From a local checkout: `openjordy plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: OpenJordy calls the same
     login endpoint, stores the access token in `~/.openjordy/credentials/matrix/credentials.json`,
...
- On first connection, OpenJordy requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  OpenJordy logs a warning.
...
`~/.openjordy/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `openjordy pairing list matrix`
  - `openjordy pairing approve matrix <CODE>`
...
- On startup, OpenJordy resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
