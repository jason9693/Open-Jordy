---
summary: "Mattermost bot setup and OpenJordy config"
read_when:
...
```bash
openjordy plugins install @openjordy/mattermost
```

Local checkout (when running from a git repo):

```bash
openjordy plugins install ./extensions/mattermost
```
...
4. Configure OpenJordy and start the gateway.
...
- Approve via:
  - `openjordy pairing list mattermost`
  - `openjordy pairing approve mattermost <CODE>`
...
Use these target formats with `openjordy message send` or cron/webhooks:
