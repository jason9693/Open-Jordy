...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects OpenJordy to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
openjordy plugins install @openjordy/feishu
```

Local checkout (when running from a git repo):

```bash
openjordy plugins install ./extensions/feishu
```
...
If you just installed OpenJordy, run the wizard:
...
2. Configuring app credentials in OpenJordy
...
1. You already ran `openjordy channels add` for Feishu
2. The gateway is running (`openjordy gateway status`)
...
```bash
openjordy pairing approve feishu <CODE>
```
...
2. Run `openjordy logs --follow` and look for `chat_id`
...
2. Run `openjordy logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `openjordy gateway status`  | Show gateway status           |
| `openjordy gateway install` | Install/start gateway service |
| `openjordy gateway stop`    | Stop gateway service          |
| `openjordy gateway restart` | Restart gateway service       |
| `openjordy logs --follow`   | Tail gateway logs             |
...
4. Check logs: `openjordy logs --follow`
...
5. Ensure the gateway is running: `openjordy gateway status`
6. Check logs: `openjordy logs --follow`
...
```json5
{
  agents: {
    list: [
      ...
      {
        id: "jordyd-fan",
        workspace: "/home/user/jordyd-fan",
        agentDir: "/home/user/.openjordy/agents/jordyd-fan/agent",
      },
      {
        id: "jordyd-xi",
        workspace: "/home/user/jordyd-xi",
        agentDir: "/home/user/.openjordy/agents/jordyd-xi/agent",
      },
    ],
  },
  bindings: [
