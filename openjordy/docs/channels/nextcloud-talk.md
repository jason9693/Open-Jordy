...
```bash
openjordy plugins install @openjordy/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
openjordy plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "OpenJordy" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `openjordy pairing list nextcloud-talk`
  - `openjordy pairing approve nextcloud-talk <CODE>`
