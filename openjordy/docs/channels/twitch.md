...
read_when:
  - Setting up Twitch chat integration for OpenJordy
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. OpenJordy connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
openjordy plugins install @openjordy/twitch
```

Local checkout (when running from a git repo):

```bash
openjordy plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `OPENJORDY_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "openjordy", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use OPENJORDY_TWITCH_ACCESS_TOKEN env var)
...
```bash
OPENJORDY_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
