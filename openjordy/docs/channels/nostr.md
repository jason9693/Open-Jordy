...
read_when:
  - You want OpenJordy to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables OpenJordy to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`openjordy onboard`) and `openjordy channels add` list optional channel plugins.
...
```bash
openjordy plugins install @openjordy/nostr
```

Use a local checkout (dev workflows):

```bash
openjordy plugins install --link <path-to-openjordy>/extensions/nostr
```
...
```json
{
  "channels": {
    "nostr": {
      ...
      "profile": {
        "name": "openjordy",
        "displayName": "OpenJordy",
        "about": "Personal assistant DM bot",
        "picture": "https://example.com/avatar.png",
        "banner": "https://example.com/banner.png",
        "website": "https://example.com",
        "nip05": "openjordy@example.com",
        "lud16": "openjordy@example.com"
      }
    }
  }
}
