...
read_when:
  - You want to place an outbound voice call from OpenJordy
...
Voice calls for OpenJordy via a plugin. Supports outbound notifications and
...
- Use `openjordy voicecall ...` or the `voice_call` tool
...
```bash
openjordy plugins install ./extensions/voice-call
