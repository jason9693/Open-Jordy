---
summary: "CLI reference for `openjordy onboard` (interactive onboarding wizard)"
read_when:
...
```bash
openjordy onboard
openjordy onboard --flow quickstart
openjordy onboard --flow manual
openjordy onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`OPENJORDY_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- Fastest first chat: `openjordy dashboard` (Control UI, no channel setup).
