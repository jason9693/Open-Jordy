---
summary: "CLI reference for `openjordy qr` (generate iOS pairing QR + setup code)"
read_when:
...
```bash
openjordy qr
openjordy qr --setup-code-only
openjordy qr --json
openjordy qr --remote
openjordy qr --url wss://gateway.example/ws --token '<token>'
```
