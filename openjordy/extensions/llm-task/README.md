...
`openjordy.invoke --each`) without adding new OpenJordy code per workflow.
...
This extension depends on OpenJordy internal modules (the embedded agent runner).
It is intended to ship as a **bundled** OpenJordy extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.openjordy/extensions` as a standalone plugin directory.
