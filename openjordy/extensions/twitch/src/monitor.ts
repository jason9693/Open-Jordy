...
import type { ReplyPayload, OpenJordyConfig } from "openjordy/plugin-sdk";
import { createReplyPrefixOptions } from "openjordy/plugin-sdk";
...
export type TwitchMonitorOptions = {
  ...
  accountId: string;
  config: unknown; // OpenJordyConfig
...
async function processTwitchMessage(params: {
  ...
  const cfg = config as OpenJordyConfig;
