...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: OPENJORDY_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: OpenJordyConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var OPENJORDY_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.OPENJORDY_TWITCH_ACCESS_TOKEN?.trim();
