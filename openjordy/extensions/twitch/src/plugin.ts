/**
 * Twitch channel plugin for OpenJordy.
 *
...
import { buildChannelConfigSchema } from "openjordy/plugin-sdk";
 ...
 * for OpenJordy. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: OpenJordyConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: OpenJordyConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: OpenJordyConfig): boolean => {
