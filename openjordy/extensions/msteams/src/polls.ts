...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["openjordyPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["openjordy", "pollId"]) ??
    readNestedString(value, ["openjordy", "poll", "id"]) ??
    readNestedString(value, ["data", "openjordyPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "openjordy", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          openjordyPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "openjordy poll vote",
          displayText: "Vote recorded",
          value: { openjordyPollId: pollId, pollId },
        },
      },
    ],
  };
