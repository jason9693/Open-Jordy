...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.OPENJORDY_STATE_DIR?.trim() || env.JORDYDBOT_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["openjordy-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".openjordy");
}
