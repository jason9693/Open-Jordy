...
export function createToolFactoryHarness(cfg: OpenJordyPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<OpenJordyPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
