import type { RuntimeEnv, ReplyPayload, OpenJordyConfig } from "openjordy/plugin-sdk";
import { createLoggerBackedRuntime, createReplyPrefixOptions } from "openjordy/plugin-sdk";
...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
  ...
  const cfg = core.config.loadConfig() as OpenJordyConfig;
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        ...
        runtime.log?.(
          `[tlon] ⚠️ SECURITY: Multiple users sharing DM session. ` +
            `Configure "session.dmScope: per-channel-peer" in OpenJordy config.`,
        );
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          const warningMsg =
            ...
            `Fix: Add to your OpenJordy config:\n` +
            `session:\n  dmScope: "per-channel-peer"\n\n` +
            `Docs: https://docs.openjordy.ai/concepts/session#secure-dm-mode`;
