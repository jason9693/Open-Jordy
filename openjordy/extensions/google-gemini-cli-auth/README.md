# Google Gemini CLI Auth (OpenJordy plugin)
...
```bash
openjordy plugins enable google-gemini-cli-auth
```
...
```bash
openjordy models auth login --provider google-gemini-cli --set-default
```
...
- `OPENJORDY_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `OPENJORDY_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
