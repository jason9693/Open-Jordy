...
function resolveGatewayPort(cfg: OpenJordyPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.OPENJORDY_GATEWAY_PORT?.trim()) ??
    parsePositiveInteger(process.env.JORDYDBOT_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
function resolveAuth(cfg: OpenJordyPluginApi["config"]): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    pickFirstDefined([
      process.env.OPENJORDY_GATEWAY_TOKEN,
      process.env.JORDYDBOT_GATEWAY_TOKEN,
      cfg.gateway?.auth?.token,
    ]) ?? undefined;
  const password =
    pickFirstDefined([
      process.env.OPENJORDY_GATEWAY_PASSWORD,
      process.env.JORDYDBOT_GATEWAY_PASSWORD,
...
async function resolveGatewayUrl(api: OpenJordyPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the OpenJordy iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the OpenJordy iOS app:",
            "",
            "```",
            qrAscii,
            "```",
