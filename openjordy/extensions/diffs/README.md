...
Read-only diff viewer plugin for **OpenJordy** agents.
...
Set plugin-wide defaults in `~/.openjordy/openjordy.json`:
...
Before:
OpenJordy supports plugins.

After:
OpenJordy supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/openjordy-diffs`).
