...
metadata: { "openjordy": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == OpenJordy `sessions_spawn`
- **File I/O** == OpenJordy `read`/`write`
- **Remote fetch** == OpenJordy `web_fetch` (or `exec` with curl when POST is required)
