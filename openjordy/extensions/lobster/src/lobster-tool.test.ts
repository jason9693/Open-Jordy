...
import type { OpenJordyPluginApi, OpenJordyPluginToolContext } from "../../../src/plugins/types.js";
import {
...
function fakeApi(overrides: Partial<OpenJordyPluginApi> = {}): OpenJordyPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<OpenJordyPluginToolContext> = {}): OpenJordyPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: OpenJordyPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
