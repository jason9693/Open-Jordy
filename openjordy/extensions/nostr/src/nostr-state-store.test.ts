...
import type { PluginRuntime } from "openjordy/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.OPENJORDY_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openjordy-nostr-"));
  process.env.OPENJORDY_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.OPENJORDY_STATE_DIR?.trim() || stateEnv.JORDYDBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".openjordy");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.OPENJORDY_STATE_DIR = previous;
    }
