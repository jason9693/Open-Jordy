...
import type { OpenJordyPluginApi, OpenJordyPluginService } from "openjordy/plugin-sdk";
...
function normalizeDenyList(cfg: OpenJordyPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: OpenJordyPluginApi["config"]): string[] {
...
export default function register(api: OpenJordyPluginApi) {
  ...
  const timerService: OpenJordyPluginService = {
