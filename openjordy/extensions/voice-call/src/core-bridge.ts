...
function resolveOpenJordyRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.OPENJORDY_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set OPENJORDY_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveOpenJordyRoot(), "dist", "extensionAPI.js");
