...
vi.mock("openjordy/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("openjordy/plugin-sdk")>("openjordy/plugin-sdk");
  return {
    ...actual,
...
import type { OpenJordyPluginServiceContext } from "openjordy/plugin-sdk";
import { emitDiagnosticEvent } from "openjordy/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/openjordy-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): OpenJordyPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("openjordy.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("openjordy.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("openjordy.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("openjordy.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("openjordy.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("openjordy.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("openjordy.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("openjordy.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("openjordy.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("openjordy.webhook.processed");
    expect(spanNames).toContain("openjordy.message.processed");
    expect(spanNames).toContain("openjordy.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["openjordy.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("openjordy.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "openjordy.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["openjordy.reason"]).toBe("string");
    expect(String(attrs?.["openjordy.reason"])).not.toContain(
