...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked OPENJORDY_HOME.
  delete process.env.OPENJORDY_HOME;
  process.env.OPENJORDY_STATE_DIR = path.join(base, ".openjordy");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "openjordy-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".openjordy", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
