...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `openjordy-e2e-${name}-`));
  const configDir = path.join(homeDir, ".openjordy");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OPENJORDY_SKIP_PROVIDERS: "1",
          OPENJORDY_SKIP_GMAIL_WATCHER: "1",
          OPENJORDY_SKIP_CRON: "1",
          OPENJORDY_SKIP_BROWSER_CONTROL_SERVER: "1",
          OPENJORDY_SKIP_CANVAS_HOST: "1",
          OPENJORDY_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
