...
process.env.OPENJORDY_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
import type { OpenJordyConfig } from "../src/config/config.js";
import type { OutboundSendDeps } from "../src/infra/outbound/deliver.js";
import { withIsolatedTestHome } from "./test-env.js";

// Set HOME/state isolation before importing any runtime OpenJordy modules.
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: OpenJordyConfig) => {
    ...
    resolveAccount: (cfg: OpenJordyConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: OpenJordyConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
