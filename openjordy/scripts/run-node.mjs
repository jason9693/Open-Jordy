...
const shouldBuild = (deps) => {
  if (deps.env.OPENJORDY_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.OPENJORDY_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[openjordy] ${message}\n`);
};

const runOpenJordy = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["openjordy.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runOpenJordy(deps);
  }
