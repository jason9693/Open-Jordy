...
BASE_IMAGE="${BASE_IMAGE:-openjordy-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-openjordy-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=openjordy.sandbox=1)
NOTE
