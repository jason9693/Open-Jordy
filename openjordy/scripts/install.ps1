# OpenJordy Installer for Windows (PowerShell)
# Usage: iwr -useb https://openjordy.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://openjordy.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\openjordy",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 OpenJordy Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one OpenJordy.$NC" -Level info
    Write-Host ""
}
...
function Install-OpenJordyNpm {
    param([string]$Version = "latest")
    
    Write-Host "Installing OpenJordy (openjordy@$Version)..." -Level info
    
    try {
        # Use -ExecutionPolicy Bypass to handle restricted execution policy
        npm install -g openjordy@$Version --no-fund --no-audit 2>&1
        Write-Host "OpenJordy installed" -Level success
        return $true
    } catch {
...
function Install-OpenJordyGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing OpenJordy from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/openjordy/openjordy.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\openjordy\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\openjordy.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install OpenJordy from git to $GitDir" -Level info
        } else {
            Install-OpenJordyGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install OpenJordy via npm (tag: $Tag)" -Level info
        } else {
            if (!(Install-OpenJordyNpm -Version $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'openjordy onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 OpenJordy installed successfully!" -Level success
}
