...
# OpenJordy Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://openjordy.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one OpenJordy."
NODE_MIN_MAJOR=22
NODE_MIN_MINOR=12
...
GUM_VERSION="${OPENJORDY_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 OpenJordy Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://openjordy.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$OPENJORDY_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.openjordy.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_openjordy_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.openjordy-* "$npm_root"/openjordy 2>/dev/null || true
}

extract_openjordy_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_openjordy_bin_conflict() {
    local bin_path="$1"
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/openjordy" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/openjordy"|"/usr/local/bin/openjordy")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/openjordy/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale openjordy symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing openjordy binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing OpenJordy package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_openjordy_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "OpenJordy npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*openjordy" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_openjordy_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_openjordy_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "OpenJordy npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an openjordy binary already exists"
            if [[ -n "$conflict" ]]; then
...
TAGLINES=()
TAGLINES+=("Your terminal just grew jordys—type something and let the bot pinch the busywork.")
...
TAGLINES+=("Automation with jordys: minimal fuss, maximal pinch.")
...
TAGLINES+=("Jordys out, commit in—let's ship something mildly responsible.")
...
HOLIDAY_CHRISTMAS="Christmas: Ho ho ho—Santa's little jordy-sistant is here to ship joy, roll back chaos, and stash the keys safely."
...
map_legacy_env "OPENJORDY_TAGLINE_INDEX" "JORDYDBOT_TAGLINE_INDEX"
map_legacy_env "OPENJORDY_NO_ONBOARD" "JORDYDBOT_NO_ONBOARD"
map_legacy_env "OPENJORDY_NO_PROMPT" "JORDYDBOT_NO_PROMPT"
map_legacy_env "OPENJORDY_DRY_RUN" "JORDYDBOT_DRY_RUN"
map_legacy_env "OPENJORDY_INSTALL_METHOD" "JORDYDBOT_INSTALL_METHOD"
map_legacy_env "OPENJORDY_VERSION" "JORDYDBOT_VERSION"
map_legacy_env "OPENJORDY_BETA" "JORDYDBOT_BETA"
map_legacy_env "OPENJORDY_GIT_DIR" "JORDYDBOT_GIT_DIR"
map_legacy_env "OPENJORDY_GIT_UPDATE" "JORDYDBOT_GIT_UPDATE"
map_legacy_env "OPENJORDY_NPM_LOGLEVEL" "JORDYDBOT_NPM_LOGLEVEL"
map_legacy_env "OPENJORDY_VERBOSE" "JORDYDBOT_VERBOSE"
map_legacy_env "OPENJORDY_PROFILE" "JORDYDBOT_PROFILE"
map_legacy_env "OPENJORDY_INSTALL_SH_NO_RUN" "JORDYDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${OPENJORDY_TAGLINE_INDEX:-}" ]]; then
        if [[ "${OPENJORDY_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((OPENJORDY_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${OPENJORDY_NO_ONBOARD:-0}
NO_PROMPT=${OPENJORDY_NO_PROMPT:-0}
DRY_RUN=${OPENJORDY_DRY_RUN:-0}
INSTALL_METHOD=${OPENJORDY_INSTALL_METHOD:-}
OPENJORDY_VERSION=${OPENJORDY_VERSION:-latest}
USE_BETA=${OPENJORDY_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/openjordy"
GIT_DIR=${OPENJORDY_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${OPENJORDY_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${OPENJORDY_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${OPENJORDY_VERBOSE:-0}"
OPENJORDY_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
OpenJordy installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://openjordy.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/openjordy)
...
Environment variables:
  OPENJORDY_INSTALL_METHOD=git|npm
  OPENJORDY_VERSION=latest|next|<semver>
  OPENJORDY_BETA=0|1
  OPENJORDY_GIT_DIR=...
  OPENJORDY_GIT_UPDATE=0|1
  OPENJORDY_NO_PROMPT=1
  OPENJORDY_DRY_RUN=1
  OPENJORDY_NO_ONBOARD=1
  OPENJORDY_VERBOSE=1
  OPENJORDY_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://openjordy.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://openjordy.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://openjordy.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected OpenJordy checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a OpenJordy source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_openjordy_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"openjordy"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://openjordy.ai/install.sh | bash"
}
...
ensure_openjordy_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/openjordy" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/openjordy" ]]; then
        ln -sf "$npm_root/openjordy/dist/entry.js" "${npm_bin}/openjordy"
        ui_info "Created openjordy bin link at ${npm_bin}/openjordy"
    fi
    return 0
}

# Check for existing OpenJordy installation
check_existing_openjordy() {
    if [[ -n "$(type -P openjordy 2>/dev/null || true)" ]]; then
        ui_info "Existing OpenJordy installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make openjordy show as \"command not found\" in new terminals."
...
warn_openjordy_not_found() {
    ui_warn "Installed, but openjordy is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t openjordy 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named openjordy; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_openjordy_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P openjordy 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/openjordy" ]]; then
        echo "${npm_bin}/openjordy"
        return 0
    fi
...
install_openjordy_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/openjordy/openjordy.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing OpenJordy from git checkout: ${repo_dir}"
    else
        ui_info "Installing OpenJordy from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning OpenJordy" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building OpenJordy" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/openjordy" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/openjordy"
    ui_success "OpenJordy wrapper installed to \$HOME/.local/bin/openjordy"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view openjordy dist-tags.beta 2>/dev/null || true)"
...
install_openjordy() {
    local package_name="openjordy"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            OPENJORDY_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="openjordy"
        else
            OPENJORDY_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${OPENJORDY_VERSION}" ]]; then
        OPENJORDY_VERSION="latest"
    fi

    local resolved_version=""
    resolved_version="$(npm view "${package_name}@${OPENJORDY_VERSION}" version 2>/dev/null || true)"
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing OpenJordy v${resolved_version}"
    else
        ui_info "Installing OpenJordy (${OPENJORDY_VERSION})"
    fi
    local install_spec=""
    if [[ "${OPENJORDY_VERSION}" == "latest" ]]; then
        install_spec="${package_name}@latest"
    else
        install_spec="${package_name}@${OPENJORDY_VERSION}"
    fi

    if ! install_openjordy_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_openjordy_paths
        install_openjordy_npm "${install_spec}"
    fi

    if [[ "${OPENJORDY_VERSION}" == "latest" && "${package_name}" == "openjordy" ]]; then
        if ! resolve_openjordy_bin &> /dev/null; then
            ui_warn "npm install openjordy@latest failed; retrying openjordy@next"
            cleanup_npm_openjordy_paths
            install_openjordy_npm "openjordy@next"
        fi
    fi

    ensure_openjordy_bin_link || true

    ui_success "OpenJordy installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local jordy="${OPENJORDY_BIN:-}"
    if [[ -z "$jordy" ]]; then
        jordy="$(resolve_openjordy_bin || true)"
    fi
    if [[ -z "$jordy" ]]; then
        ui_info "Skipping doctor (openjordy not on PATH yet)"
        warn_openjordy_not_found
        return 0
    fi
    run_quiet_step "Running doctor" "$jordy" doctor --non-interactive || true
...
maybe_open_dashboard() {
    ...
    if ! "$jordy" dashboard --help >/dev/null 2>&1; then
        return 0
    fi
    "$jordy" dashboard || true
}

resolve_workspace_dir() {
    local profile="${OPENJORDY_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.openjordy/workspace-${profile}"
    else
        echo "${HOME}/.openjordy/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${OPENJORDY_CONFIG_PATH:-$HOME/.openjordy/openjordy.json}"
    if [[ -f "${config_path}" || -f "$HOME/.jordydbot/jordydbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
        return
    fi
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run openjordy onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$jordy" ]]; then
        ui_info "BOOTSTRAP.md found but openjordy not on PATH; skipping onboarding"
        warn_openjordy_not_found
        return
    fi

    "$jordy" onboard || {
        ui_error "Onboarding failed; run openjordy onboard to retry"
        return
    }
}

resolve_openjordy_version() {
    local version=""
    local jordy="${OPENJORDY_BIN:-}"
    if [[ -z "$jordy" ]] && command -v openjordy &> /dev/null; then
        jordy="$(command -v openjordy)"
    fi
    if [[ -n "$jordy" ]]; then
        version=$("$jordy" --version 2>/dev/null | head -n 1 | tr -d '\r')
    fi
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/openjordy/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/openjordy/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
is_gateway_daemon_loaded() {
    ...
    local status_json=""
    status_json="$("$jordy" daemon status --json 2>/dev/null || true)"
...
refresh_gateway_service_if_loaded() {
    ...
    if ! is_gateway_daemon_loaded "$jordy"; then
        return 0
    fi

    ui_info "Refreshing loaded gateway service"
    if run_quiet_step "Refreshing gateway service" "$jordy" gateway install --force; then
    ...
    if run_quiet_step "Restarting gateway service" "$jordy" gateway restart; then
    ...
    run_quiet_step "Probing gateway service" "$jordy" gateway status --probe --deep || true
}
...
main() {
    ...
    detected_checkout="$(detect_openjordy_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found OpenJordy checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set OPENJORDY_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_openjordy; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing OpenJordy"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g openjordy &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g openjordy 2>/dev/null || true
        ...
        install_openjordy_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/openjordy" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/openjordy"
    ...
    OPENJORDY_BIN="$(resolve_openjordy_bin || true)"
    ...
    installed_version=$(resolve_openjordy_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 OpenJordy installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 OpenJordy installed successfully!"
    fi
    if [[ "$is_upgrade" == "true" ]]; then
        local update_messages=(
            ...
            "The lobster has molted. Harder shell, sharper jordys."
        ...
        local completion_messages=(
            ...
            "cracks jordys Alright, what are we building?"
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/openjordy"
        ui_kv "Update command" "openjordy update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://openjordy.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            local -a doctor_args=()
            if [[ "$NO_ONBOARD" == "1" ]]; then
                if "$jordy" doctor --help 2>/dev/null | grep -q -- "--non-interactive"; then
                    doctor_args+=("--non-interactive")
                fi
            fi
            ui_info "Running openjordy doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                OPENJORDY_UPDATE_IN_PROGRESS=1 "$jordy" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                OPENJORDY_UPDATE_IN_PROGRESS=1 "$jordy" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                OPENJORDY_UPDATE_IN_PROGRESS=1 "$jordy" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run openjordy doctor and openjordy plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run openjordy onboard later"
        else
            local config_path="${OPENJORDY_CONFIG_PATH:-$HOME/.openjordy/openjordy.json}"
            if [[ -f "${config_path}" || -f "$HOME/.jordydbot/jordydbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$jordy" ]]; then
                    ui_info "Skipping onboarding (openjordy not on PATH yet)"
                    warn_openjordy_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run openjordy onboard to finish setup"
            return 0
        fi
    fi

    if command -v openjordy &> /dev/null; then
        local jordy="${OPENJORDY_BIN:-}"
        if [[ -z "$jordy" ]]; then
            jordy="$(resolve_openjordy_bin || true)"
        fi
        if [[ -n "$jordy" ]] && is_gateway_daemon_loaded "$jordy"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (openjordy daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if OPENJORDY_UPDATE_IN_PROGRESS=1 "$jordy" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: openjordy daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${OPENJORDY_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
