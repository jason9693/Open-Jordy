...
# OpenJordy Auth Widget for Termux
...
SERVER="${OPENJORDY_SERVER:-${JORDYDBOT_SERVER:-l36}}"

# Check auth status
termux-toast "Checking OpenJordy auth..."

STATUS=$(ssh "$SERVER" '$HOME/openjordy/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/openjordy/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|OPENJORDY_EXPIRING|JORDYDBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/openjordy/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|OPENJORDY_EXPIRED|JORDYDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: OpenJordy won't work until re-auth"
                ;;
        esac
        ;;
