...
function stripWorkspaceOpenjordyDevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.openjordy !== "workspace:*") {
    return false;
  }
  delete devDeps.openjordy;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceOpenjordyDevDependency(pkg);
