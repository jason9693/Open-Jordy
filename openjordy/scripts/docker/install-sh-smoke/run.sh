...
INSTALL_URL="${OPENJORDY_INSTALL_URL:-https://openjordy.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${OPENJORDY_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${OPENJORDY_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="openjordy"
PACKAGE_NAME="${OPENJORDY_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (OPENJORDY_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${OPENJORDY_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${OPENJORDY_INSTALL_LATEST_OUT:-}"
fi
