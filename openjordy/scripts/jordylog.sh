...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "jordylog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
jordylog - OpenJordy Logging Utility

USAGE:
    jordylog [OPTIONS]

DESCRIPTION:
    View OpenJordy logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    OpenJordy logs flow through the macOS unified log (subsystem: ai.openjordy).
...
QUICK START:
    jordylog -n 100             Show last 100 lines from all components
    jordylog -f                 Follow logs in real-time
    jordylog -e                 Show only errors
    jordylog -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    jordylog                   Show last 50 lines from past 5 minutes (default)
    jordylog -f                Stream logs continuously
    jordylog -n 100            Show last 100 lines
    jordylog -e                Show only recent errors
    jordylog -l 30m -n 200     Show last 200 lines from past 30 minutes
    jordylog -c ServerManager  Show recent ServerManager logs
    jordylog -s "fail"         Search for "fail" in recent logs
    jordylog --server -e       Show recent server errors
    jordylog -f -d             Stream debug logs continuously
