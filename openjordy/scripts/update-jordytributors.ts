...
import type { ApiContributor, Entry, MapConfig, User } from "./update-jordytributors.types.js";

const REPO = "openjordy/openjordy";
const PER_LINE = 10;

const mapPath = resolve("scripts/jordytributors-map.json");
...
if (start === -1 || end === -1) {
  throw new Error("README.md missing jordytributors block");
}

const next = `${readme.slice(0, start)}<p align="left">\n${block}${readme.slice(end)}`;
writeFileSync(readmePath, next);

console.log(`Updated README jordytributors: ${entries.length} entries`);
