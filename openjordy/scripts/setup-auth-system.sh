#!/bin/bash
# Setup OpenJordy Auth Management System
...
echo "=== OpenJordy Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'openjordy-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  OpenJordy message: Send warning via OpenJordy itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/openjordy-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/openjordy-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/openjordy-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now openjordy-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/JordydAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/JordydAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Jordyd*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (JordydAuth) shows status and opens auth URL if needed."
echo "The full widget (JordydAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status openjordy-auth-monitor.timer"
echo ""
echo "Quick commands:"
