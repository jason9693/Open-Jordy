...
#
# Suggested cron: */30 * * * * /home/admin/openjordy/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send OpenJordy notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., openjordy-alerts)
...
STATE_FILE="$HOME/.openjordy/auth-monitor-state"
...
send_notification() {
    ...
    # Send via OpenJordy if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use openjordy
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via OpenJordy to $NOTIFY_PHONE..."
            openjordy send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! OpenJordy is down. Run: ssh l36 '~/openjordy/scripts/mobile-reauth.sh'" "urgent"
    exit 1
