...
IMAGE_NAME="openjordy-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="openjordy-net-e2e-$$"
GW_NAME="openjordy-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "OPENJORDY_GATEWAY_TOKEN=$TOKEN" \
  -e "OPENJORDY_SKIP_CHANNELS=1" \
  -e "OPENJORDY_SKIP_GMAIL_WATCHER=1" \
  -e "OPENJORDY_SKIP_CRON=1" \
  -e "OPENJORDY_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
