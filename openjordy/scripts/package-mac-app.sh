...
# Build and bundle OpenJordy into a minimal .app we can open.
# Outputs to dist/OpenJordy.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/OpenJordy.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="OpenJordy"
BUNDLE_ID="${BUNDLE_ID:-ai.openjordy.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/openjordy/openjordy/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/OpenJordy/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :OpenJordyBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OpenJordyGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/OpenJordy"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/OpenJordy"
fi
chmod +x "$APP_ROOT/Contents/MacOS/OpenJordy"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/OpenJordy" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/OpenJordy/Resources/OpenJordy.icns" "$APP_ROOT/Contents/Resources/OpenJordy.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/OpenJordy/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OpenJordyKit resources"
OPENJORDYKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OpenJordyKit_OpenJordyKit.bundle"
if [ -d "$OPENJORDYKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OpenJordyKit_OpenJordyKit.bundle"
  cp -R "$OPENJORDYKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OpenJordyKit_OpenJordyKit.bundle"
else
  echo "WARN: OpenJordyKit resource bundle not found at $OPENJORDYKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running OpenJordy"
killall -q OpenJordy 2>/dev/null || true
