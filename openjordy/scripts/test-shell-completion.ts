 ...
 * `openjordy update`. Use it to verify the completion installation flow
...
\`openjordy update\`, \`openjordy doctor\`, and \`openjordy onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# OpenJordy Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`openjordy completion --install\` later to enable.`));
    return;
  }
