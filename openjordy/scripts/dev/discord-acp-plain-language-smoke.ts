...
type DriverMode = "token" | "webhook" | "openjordy";
...
function resolveStateDir(): string {
  const override = process.env.OPENJORDY_STATE_DIR?.trim() || process.env.JORDYDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.OPENJORDY_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".openjordy");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver openjordy] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) OpenJordy spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|openjordy> Driver transport mode (default: token)\n" +
    ...
    "  --openjordy-bin <path>        OpenJordy CLI binary for driver=openjordy (default: openjordy)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  OPENJORDY_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  OPENJORDY_DISCORD_SMOKE_DRIVER\n" +
    "  OPENJORDY_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  OPENJORDY_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  OPENJORDY_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  OPENJORDY_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  OPENJORDY_DISCORD_SMOKE_AGENT\n" +
    "  OPENJORDY_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  OPENJORDY_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  OPENJORDY_DISCORD_SMOKE_POLL_MS\n" +
    "  OPENJORDY_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  OPENJORDY_DISCORD_SMOKE_OPENJORDY_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.OPENJORDY_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.JORDYDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.OPENJORDY_DISCORD_SMOKE_DRIVER ||
    process.env.JORDYDBOT_DISCORD_SMOKE_DRIVER ||
    "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "openjordy"
        ? "openjordy"
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.OPENJORDY_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.JORDYDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.OPENJORDY_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.OPENJORDY_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.JORDYDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.OPENJORDY_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.OPENJORDY_DISCORD_SMOKE_AGENT ||
    process.env.JORDYDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.OPENJORDY_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.JORDYDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.OPENJORDY_DISCORD_SMOKE_INSTRUCTION ||
    process.env.JORDYDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.OPENJORDY_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.OPENJORDY_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.OPENJORDY_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const openjordyBin =
    resolveArg("--openjordy-bin") || process.env.OPENJORDY_DISCORD_SMOKE_OPENJORDY_BIN || "openjordy";
...
async function openjordyCliJson<T>(params: { openjordyBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.openjordyBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`openjordy ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithOpenjordy(params: {
  openjordyBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await openjordyCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    openjordyBin: params.openjordyBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "openjordy") {
    return await readMessagesWithOpenjordy({
      openjordyBin: params.args.openjordyBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `openjordy-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await openjordyCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
        ...
        openjordyBin: args.openjordyBin,
        args: [
          "message",
          "send",
          "--channel",
      ...
      if (!sentMessageId) {
        throw new Error("openjordy message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "openjordy"
            ? await readMessagesWithOpenjordy({
                openjordyBin: args.openjordyBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from OpenJordy.`,
        diagnostics: {
          bindingCandidates: [
            {
