...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/openjordy/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "OpenJordy Re-Auth" -c "After getting key, run: ssh $SERVER '~/openjordy/scripts/mobile-reauth.sh'" --id openjordy-auth
        ;;
    *)
